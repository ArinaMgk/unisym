...
\item \verb|for(dec_or_calls_div_by_commas; cond; exps0) exps;| $\leftrightarrow$ \verb|dec_or_calls_div_by_commas; while(cond){exps;exps0;}|
\item \verb|while(condition) {functions(arguments);}| $\leftrightarrow$ \verb|a:if(condition)goto b; /***/ goto a;b:;| $\leftrightarrow$ \verb|while:condition; JZ endowhile; ...; JMP while; endowhile:;|
\item Single definition check \verb|#ifdef xxx| $\simeq$ \verb|#if defined(xxx)|
\begin{itemize}

\item Similarly, \verb|#ifndef xxx| $\simeq$ \verb|#if !defined(xxx)|
\end{itemize}
...
\item This is also true for floating numbers and pointers. (test yo RFW14, MSVC2022)
\end{itemize}
...
\subsubsection{Definition and Declaration}
...
\textbf{EXAMPLE for }\textbf{\textit{MASM}} - Print 10 numbers
...
The macro \texttt{for0}, \texttt{for1}, \texttt{for0a} and more, are defined in \verb|alice.h| (used by \verb|stdinc.h|) for C and C++.
