// ASCII C TAB4 CRLF
// Docutitle: (Format.Executable) Object Module Format
// Codifiers: @ArinaMgk
...
static ObjRecord *obj_new(void)
{
    ...
    orp = obj_clear(malcof(ObjRecord));
    orp->ori = ori_null;
    return (orp);
}
...
static ObjRecord *obj_bump(ObjRecord * orp)
{
    ...
    if (orp->up) {
        *orp->up = nxt = obj_new();
        ...
        nxt->up = orp->up;
        nxt->back = orp;
        MemCopyN(nxt->parm, orp->parm, sizeof(orp->parm));
    } else
        nxt = obj_emit(orp);
    ...
    if (used) {
        ...
        MemCopyN(nxt->buf + nxt->committed, orp->buf + committed, used);
...
static ObjRecord* obj_name(ObjRecord* orp, const char* name)
{
    ...
    *ptr++ = len;
    ...
    if (obj_uppercase)
        while (--len >= 0) {
            *ptr++ = ascii_toupper(*name);
            name++;
        }
    else
        MemCopyN(ptr, name, len);
    return (orp);
}
...
static int32_t obj_segment(rostr, int, int*);
static void obj_write_file(int debuginfo);
static int obj_directive(char*, char*, int);

void dbgbi_init(outffmt* of, void* id, FILE* fp);
static void dbgbi_linnum(const char* lnfname, int32_t lineno, int32_t segto);
static void dbgbi_deflabel(char *name, int32_t segment, int64_t offset, int is_global, char *special);
static void dbgbi_typevalue(int32_t type);
static void dbgbi_output(int output_type, void *param);
static void dbgbi_cleanup();
static void obj_init(FILE *fp, ldfunc ldef, evalfunc eval);
static int obj_set_info(geninfo type, char **val);
static void obj_out(int32_t segto, const void *data, out_type type, uint64_t size, int32_t segment, int32_t wrt);
static void obj_deflabel(char *name, int32_t segment, int64_t offset, int is_global, char *special);
...
_ESYM_C macros_t obj_stdmac[];


static struct dbgffmt borland_debug_form = {
...
static struct dbgffmt* borland_debug_arr[3] = {
    &borland_debug_form,
    &null_debug_form,
    NULL
};

struct outffmt of_obj = {
...
static void obj_cleanup(int debuginfo)
{
    ...
    while (imphead) {
        ...
        memf(imptmp->extname);
        memf(imptmp->libname);
        memf(imptmp->impname);     /* free won't mind if it's NULL */
        memf(imptmp);
    }
    while (exphead) {
...
static void obj_ext_set_defwrt(struct External *ext, char *id)
{
    ...
    for (seg = seghead; seg; seg = seg->next)
        if (!StrCompare(seg->name, id)) {
            ext->defwrt_type = External::DEFWRT_SEGMENT;
    ...
    for (grp = grphead; grp; grp = grp->next)
        if (!StrCompare(grp->name, id)) {
            ext->defwrt_type = External::DEFWRT_GROUP;
            ext->defwrt_ptr.grp = grp;
            memf(id);
            return;
        }

    ext->defwrt_type = External::DEFWRT_STRING;
                         ...
                         int64_t offset, int is_global, char *special)
{
    ...
    if (segment >= SEG_ABS || segment == NO_SEG) {
        ...
        if (is_global) {
            ...
            pub = *fpubtail = (Public*)malc(sizeof(*pub));
            fpubtail = &pub->next;
            pub->next = NULL;
            pub->name = StrHeap(name);
            pub->offset = offset;
            pub->segment = (segment == NO_SEG ? 0 : segment & ~SEG_ABS);
        }
        if (special)
            aasm_log(_LOG_ERROR, "OBJ supports no special symbol features"
    ...
    if (!any_segs && segment == first_seg) {
        ...
        if (segment != obj_segment("__NASMDEFSEG", 2, &tempint))
            aasm_log(_LOG_PANIC, "strange segment conditions in OBJ driver");
    }

    for (seg = seghead; seg && is_global; seg = seg->next)
        if (seg->index == segment) {
            Public *loc = (Public*)malc(sizeof(*loc));
            /*
     ...
     */
    if (is_global) {
        ext = *exttail = (External*)malc(sizeof(*ext));
        ext->next = NULL;
        exttail = &ext->next;
        ext->name = name;
        /* Place by default all externs into the current segment */
        ext->defwrt_type = External::DEFWRT_NONE;
...
*/
    ...
    while (special && *special) {
         ...
         */
        if (!StrCompareNInsensitive(special, "wrt", 3)) {
            char *p;
            int len;
            special += 3;
            special += StrSpanInclude(special, " \t");
            p = StrHeapN(special, len = StrSpanExclude(special, ":"));
            obj_ext_set_defwrt(ext, p);
            special += len;
            if (*special && *special != ':')
                aasm_log(_LOG_ERROR, "`:' expected in special symbol"
                      " text for `%s'", ext->name);
            else if (*special == ':')
                special++;
        }
         ...
         */
        if (!StrCompareNInsensitive(special, "far", 3)) {
            if (ext->commonsize)
                ext->commonelem = 1;
            else
                aasm_log(_LOG_ERROR,
            ...
            special += 3;
            special += StrSpanInclude(special, " \t");
        } else if (!StrCompareNInsensitive(special, "near", 4)) {
            if (ext->commonsize)
                ext->commonelem = 0;
            else
                aasm_log(_LOG_ERROR,
    ...
    eb = ebhead;
    if (!eb) {
        eb = *ebtail = (ExtBack*)malc(sizeof(*eb));
        eb->next = NULL;
        ebtail = &eb->next;
    }
    while (i >= EXT_BLKSIZ) {
                    ...
                    int32_t segment, int32_t wrt)
{
    ...
    if (type == OUT_RAWDATA) {
        ...
        while (size > 0) {
            ...
            if (len > size)
                len = size;
            MemCopyN(orp->buf + orp->used, ucdata, len);
        ...
        if (segment == NO_SEG && type != OUT_ADDRESS)
            aasm_log(_LOG_ERROR, "relative call to absolute address not"
                  " supported by OBJ format");
        if (segment >= SEG_ABS)
            aasm_log(_LOG_ERROR, "far-absolute relocations not supported"
                  " by OBJ format");
        ldata = *(int64_t *)data;
        if (type == OUT_REL2ADR) {
            ldata += (size - 2);
        ...
        if (segment < SEG_ABS && (segment != NO_SEG && segment % 2) &&
            ...
            rsize = 2;
            if (ldata & 0xFFFF)
                aasm_log(_LOG_ERROR, "OBJ format cannot handle complex"
                            ...
                            struct Segment *segto)
{
    ...
    if (bytes == 1) {
        aasm_log(_LOG_ERROR, "`obj' output driver does not support"
    ...
    if (seg % 2) {
        ...
        seg--;
        if (bytes != 2)
            aasm_log(_LOG_PANIC, "OBJ: 4-byte segment base fixup got"
    ...
    if (wrt == NO_SEG) {
        ...
        else if (!base && e && e->defwrt_type != External::DEFWRT_NONE) {
            if (e->defwrt_type == External::DEFWRT_SEGMENT)
                method |= 0x00, fidx = e->defwrt_ptr.seg->obj_index;
            else if (e->defwrt_type == External::DEFWRT_GROUP)
                method |= 0x10, fidx = e->defwrt_ptr.grp->obj_index;
            else {
                aasm_log(_LOG_ERROR, "default WRT specification for"
...
static int32_t obj_segment(rostr name, int pass, int *bits)
{
    /*
	...
	bool rn_error = 0;
        ...
        for (seg = seghead; seg; seg = seg->next) {
            obj_idx++;
            if (!StrCompare(seg->name, name)) {
                if (attrs > 0 && pass == 1)
                    aasm_log(_LOG_WARN, "segment attributes specified on"
        ...
        *segtail = seg = (Segment*)malc(sizeof(*seg));
        seg->next = NULL;
        segtail = &seg->next;
        seg->index = (any_segs ? SegAlloc() : first_seg);
        ...
        seg->combine = Segment::CMB_PUBLIC;      /* default */
        ...
        while (attrs--) {
             ...
             */
            if (!StrCompareInsensitive(p, "private"))
                seg->combine = Segment::CMB_PRIVATE;
            else if (!StrCompareInsensitive(p, "public"))
                seg->combine = Segment::CMB_PUBLIC;
            else if (!StrCompareInsensitive(p, "common"))
                seg->combine = Segment::CMB_COMMON;
            else if (!StrCompareInsensitive(p, "stack"))
                seg->combine = Segment::CMB_STACK;
            else if (!StrCompareInsensitive(p, "use16"))
                seg->use32 = false;
            else if (!StrCompareInsensitive(p, "use32"))
                seg->use32 = true;
            else if (!StrCompareInsensitive(p, "flat")) {
                /*
                ...
                struct Group *grp;
                for (grp = grphead; grp; grp = grp->next)
                    if (!StrCompare(grp->name, "FLAT"))
                        break;
                if (!grp) {
                    obj_directive((char*)"group", (char*)"FLAT", 1);
                    for (grp = grphead; grp; grp = grp->next)
                        if (!StrCompare(grp->name, "FLAT"))
                            break;
                    if (!grp)
                        aasm_log(_LOG_PANIC, "failure to define FLAT?!");
                }
                seg->grp = grp;
            } else if (!StrCompareNInsensitive(p, "class=", 6))
                seg->segclass = StrHeap(p + 6);
            else if (!StrCompareNInsensitive(p, "overlay=", 8))
                seg->overlay = StrHeap(p + 8);
            else if (!StrCompareNInsensitive(p, "align=", 6)) {
                seg->align = readnum(p + 6, &rn_error);
                if (rn_error) {
                    seg->align = 1;
                    aasm_log(_LOG_ERROR, "segment alignment should be"
                          " numeric");
                }
                ...
                case 2048:
                    ...
                    break;
                default:
                    aasm_log(_LOG_ERROR, "invalid alignment value %d",
                          seg->align);
                    seg->align = 1;
                    break;
                }
            } else if (!StrCompareNInsensitive(p, "absolute=", 9)) {
                seg->align = SEG_ABS + readnum(p + 9, &rn_error);
                if (rn_error)
                    aasm_log(_LOG_ERROR, "argument to `absolute' segment"
        ...
        for (grp = grphead; grp; grp = grp->next) {
            for (i = grp->nindices; i < grp->nentries; i++) {
                if (!StrCompare(grp->segs[i].name, seg->name)) {
                    memf(grp->segs[i].name);
        ...
        extp = &dws;
        while (*extp) {
            if ((*extp)->defwrt_type == External::DEFWRT_STRING &&
                !StrCompare((*extp)->defwrt_ptr.string, seg->name)) {
                memf((*extp)->defwrt_ptr.string);
                (*extp)->defwrt_type = External::DEFWRT_SEGMENT;
...
static int obj_directive(char *directive, char *value, int pass)
{
    if (!StrCompare(directive, "group")) {
        ...
        if (pass == 1) {
             ...
             *
             * if (!*q) {
             *     aasm_log(_LOG_ERROR,"GROUP directive contains no segments");
             *     return 1;
             * }
             */
            ...
            for (grp = grphead; grp; grp = grp->next) {
                obj_idx++;
                if (!StrCompare(grp->name, v)) {
                    aasm_log(_LOG_ERROR, "group `%s' defined twice", v);
                    return 1;
                }
            }

            *grptail = grp = (Group*)malc(sizeof(*grp));
            grp->next = NULL;
            grptail = &grp->next;
            grp->index = SegAlloc();
            ...
            while (*q) {
                ...
                if (seg) {
                     ...
                     */
                    grp->segs[grp->nentries++].name = StrHeap(p);
                }
            }
            ...
            while (*extp) {
                if ((*extp)->defwrt_type == External::DEFWRT_STRING &&
                    !StrCompare((*extp)->defwrt_ptr.string, grp->name)) {
                    memf((*extp)->defwrt_ptr.string);
                    (*extp)->defwrt_type = External::DEFWRT_GROUP;
    ...
    if (!StrCompare(directive, "uppercase")) {
        obj_uppercase = true;
        return 1;
    }
    if (!StrCompare(directive, "import")) {
        ...
        if (!*extname || !*libname)
            aasm_log(_LOG_ERROR, "`import' directive requires symbol name"
                  " and library name");
        else {
            struct ImpDef *imp;
            bool err = false;

            imp = *imptail = malcof(struct ImpDef);
            imptail = &imp->next;
            imp->next = NULL;
            imp->extname = StrHeap(extname);
            imp->libname = StrHeap(libname);
            imp->impindex = readnum(impname, &err);
            if (!*impname || err)
                imp->impname = StrHeap(impname);
            else
                imp->impname = NULL;
        }
    ...
    if (!StrCompare(directive, "export")) {
        char *q, *extname, *intname, *v;
        struct ExpDef *export_def;
        int flags = 0;
        ...
        if (!*intname) {
            aasm_log(_LOG_ERROR, "`export' directive requires export name");
            return 1;
        }
        if (!*extname) {
            extname = intname;
        ...
        while (*q) {
            ...
            if (!StrCompareInsensitive(v, "resident"))
                flags |= EXPDEF_FLAG_RESIDENT;
            else if (!StrCompareInsensitive(v, "nodata"))
                flags |= EXPDEF_FLAG_NODATA;
            else if (!StrCompareNInsensitive(v, "parm=", 5)) {
                bool err = false;
        ...
        export_def = *exptail = malcof(struct ExpDef);
        exptail = &export_def->next;
        export_def->next = NULL;
        export_def->extname = StrHeap(extname);
        export_def->intname = StrHeap(intname);
        export_def->ordinal = ordinal;
        export_def->flags = flags;
...
static int32_t obj_segbase(int32_t segment)
{
    ...
    if (!seg) {
        ...
        if (eb) {
            ...
            case External::DEFWRT_NONE:
                return segment; /* fine */
            case External::DEFWRT_SEGMENT:
                return e->defwrt_ptr.seg->index + 1;
            case External::DEFWRT_GROUP:
...
static void obj_write_file(int debuginfo)
{
     ...
     */
    for (export_def = exphead; export_def; export_def = export_def->next) {
        obj_rword(orp, 0xA0);   /* comment class A0 */
        obj_byte(orp, 2);       /* subfunction 2: EXPDEF */
        obj_byte(orp, export_def->flags);
        obj_name(orp, export_def->extname);
        obj_name(orp, export_def->intname);
        if (export_def->flags & EXPDEF_FLAG_ORDINAL)
            obj_word(orp, export_def->ordinal);
        obj_emit2(orp);
    }
    ...
    for (seg = seghead; seg; seg = seg->next) {
        ...
        if (seg->align >= SEG_ABS)
            ...
            if (seg->align > 4096)
                aasm_log(_LOG_ERROR, "segment `%s' requires more alignment"
    ...
    for (grp = grphead; grp; grp = grp->next) {
        ...
        if (grp->nindices != grp->nentries) {
            for (i = grp->nindices; i < grp->nentries; i++) {
                aasm_log(_LOG_ERROR, "group `%s' contains undefined segment"
    ...
    if (obj_entry_seg != NO_SEG) {
        ...
        if (!seg)
            aasm_log(_LOG_ERROR, "entry point is not in this module");
    }
...
void dbgbi_init(struct outffmt *of, void *id, FILE * fp)
{
    (void)of;
    (void)id;
    (void)fp;
...
static void dbgbi_linnum(const char *lnfname, int32_t lineno, int32_t segto)
{
    ...
    for (seg = seghead; seg; seg = seg->next)
        if (seg->index == segto)
            break;
    if (!seg)
        aasm_log(_LOG_PANIC, "lineno directed to nonexistent segment?");

/*    for (fn = fnhead; fn; fn = fnhead->next) */
    for (fn = fnhead; fn; fn = fn->next)        /* fbk - Austin Lunnen - John Fine */
        if (!StrCompareInsensitive(lnfname, fn->name))
            break;
    if (!fn) {
        fn = (FileName*)malc(sizeof(*fn));
        fn->name = (char*)malc(StrLength(lnfname) + 1);
        StrCopy(fn->name, lnfname);
        fn->lnhead = NULL;
        fn->lntail = &fn->lnhead;
        fn->next = NULL;
        *fntail = fn;
...
static void dbgbi_typevalue(int32_t type)
{
    ...
    if (elem > 1) {
        Array *arrtmp = (Array*)malc(sizeof(*arrtmp));
