...
int32_t disasm(uint8_t *data, char *output, int outbufsize, int segsize, int32_t offset, int autosync, uint32_t prefer);
...
static enum reg_enum whichreg(int32_t regflags, int regval, int rex)
{
	...
	return R_YMM0;
	if (!(REG_CS & ~regflags))
		return (regval == 1) ? R_CS : R_zero;
	if (!(REG_DESS & ~regflags))
		return (regval == 0 || regval == 2
				|| regval == 3 ? aasm_rd_sreg[regval] : R_zero);
	if (!(REG_FSGS & ~regflags))
		return (regval == 4 || regval == 5 ? aasm_rd_sreg[regval] : R_zero);
	if (!(REG_SEG67 & ~regflags))
		return (regval == 6 || regval == 7 ? aasm_rd_sreg[regval] : R_zero);
	...
	if (!(REG8 & ~regflags)) {
	if (rex & (REX_P|REX_NH))
		return aasm_rd_reg8_rex[regval];
	else
		return aasm_rd_reg8[regval];
	}
	if (!(REG16 & ~regflags))
		return aasm_rd_reg16[regval];
	if (!(REG32 & ~regflags))
		return aasm_rd_reg32[regval];
	if (!(REG64 & ~regflags))
		return aasm_rd_reg64[regval];
	if (!(REG_SREG & ~regflags))
		return aasm_rd_sreg[regval & 7]; /* Ignore REX */
	if (!(REG_CREG & ~regflags))
		return aasm_rd_creg[regval];
	if (!(REG_DREG & ~regflags))
		return aasm_rd_dreg[regval];
	if (!(REG_TREG & ~regflags)) {
	if (regval > 7)
		return R_zero;		/* TR registers are ill-defined with rex */
		return aasm_rd_treg[regval];
	}
	if (!(FPUREG & ~regflags))
		return aasm_rd_fpureg[regval & 7]; /* Ignore REX */
	if (!(MMXREG & ~regflags))
		return aasm_rd_mmxreg[regval & 7]; /* Ignore REX */
	if (!(XMMREG & ~regflags))
		return aasm_rd_xmmreg[regval];
	if (!(YMMREG & ~regflags))
		return aasm_rd_ymmreg[regval];
	return R_zero;
}
			  ...
			  int segsize, operand * op, insn *ins)
{
	...
	if (mod == 3) {             /* pure register version */
		op->basereg = (reg_enum)(rm + (rex & REX_B ? 8 : 0));
	...
	if (asize == 16) {
		 ...
		 */
		op->indexreg = op->basereg = (reg_enum)-1;
	...
	if (a64)
		op->basereg = aasm_rd_reg64[rm | ((rex & REX_B) ? 8 : 0)];
	else
		op->basereg = aasm_rd_reg32[rm | ((rex & REX_B) ? 8 : 0)];
		...
		op->indexreg = aasm_rd_reg64[index | ((rex & REX_X) ? 8 : 0)];
		else
		op->indexreg = aasm_rd_reg32[index | ((rex & REX_X) ? 8 : 0)];

		if (base == 5 && mod == 0) {
		op->basereg = R_none;
		mod = 2;	/* Fake disp32 */
		} else if (a64)
		op->basereg = aasm_rd_reg64[base | ((rex & REX_B) ? 8 : 0)];
		else
		op->basereg = aasm_rd_reg32[base | ((rex & REX_B) ? 8 : 0)];
		   ...
		   const struct prefix_info *prefix, int segsize, insn *ins)
{
	...
	case4(010):
	{
			...
			if (d < t || d > t + 7)
				...
				opx->basereg = (reg_enum)((d - t) + (ins->rex & REX_B ? 8 : 0));
	...
	case4(0130):
	{
		...
		if (!data)
		return false;
		opx->basereg = (reg_enum)(((modrm >> 3) & 7) + (ins->rex & REX_R ? 8 : 0));
		break;
	}
	...
	case 0172:
	{
		uint8_t ximm = *data++;
		c = *r++;
		ins->oprs[c >> 3].basereg = (reg_enum)((ximm >> 4) & regmask);
	...
	case 0173:
	{
		...
		ins->oprs[c >> 4].basereg = (reg_enum)((ximm >> 4) & regmask);
	...
	case 0174:
	{
		...
		ins->oprs[c].basereg = (reg_enum)((ximm >> 4) & regmask);
	...
	case4(0260):
	{
		...
		opx->segment |= SEG_RMREG;
		opx->basereg = (reg_enum)prefix->vex_v;
		vex_ok = true;
		break;
	}
	...
	case4(0344):
		ins->oprs[0].basereg = reg_enum((*data++ >> 3) & 7);
		break;
			...
			int32_t offset, int autosync, uint32_t prefer)
{
	...
	i = (*p)->opcode;
	if (i >= _AASM_INST_FIRST_COND_OPCODE)
	slen += snprintf(output + slen, outbufsize - slen, "%s%s",
			 _asm_inst_literal_list[i], condition_name[ins.condition]);
	else
		slen += snprintf(output + slen, outbufsize - slen, "%s",
			 _asm_inst_literal_list[i]);
	...
	offs = o->offset;
		...
		enum reg_enum reg;
			...
			slen += snprintf(output + slen, outbufsize - slen, "%s",
							 aasm_reg_names[reg-EXPR_REG_START]);
		} else if (!(UNITY & ~t)) {
			output[slen++] = '1';
		} else if (t & IMMEDIATE) {
			if (t & BITS8) {
			...
			slen += snprintf(output + slen, outbufsize - slen, "0x%" PRIx64 "", offs);
		} else if (!(MEM_OFFS & ~t)) {
			slen +=
		...
		slen += snprintf(output + slen, outbufsize - slen, "rel ");
			...
			if (o->basereg != -1) {
				slen += snprintf(output + slen, outbufsize - slen, "%s",
								 aasm_reg_names[(o->basereg-EXPR_REG_START)]);
				started = true;
			}
			if (o->indexreg != -1) {
				if (started)
					output[slen++] = '+';
				slen += snprintf(output + slen, outbufsize - slen, "%s",
								 aasm_reg_names[(o->indexreg-EXPR_REG_START)]);
				if (o->scale > 1)
					slen +=
