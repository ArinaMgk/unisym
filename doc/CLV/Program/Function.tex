We usually consider procedure or sub-routine only exist in Assembly. Someone call the function returning nothing procedure.
...
Because pointers are in a size, it is OK to write \verb|f(int arr[1][2]) {...}| as \verb|f(int arr[][2]) { ... }| or \verb|f(int (*arr)[2]) { ... }|. For multi arguments, the evaluation order (before calling) is decided by the compiler.
...
The \textbf{storage-class specifier} for a parameter declaration is only \verb|register|.



\subsubsection{Calling Convention - Prologue and Epilogue}

\labex{Calling}

\textbf{Header}: auto-inc-by stdinc.

The registers that need to be preserved before calling a function and stored after its returning is \textbf{Function Calling Context}. The saving subject is whether caller or callee.
...
\textbf{Register BP for Entering and Leaving Function}

\textbf{Allocation of Stack}
...
This appeared when the keyword \refer{keyword.inline} appeared. In a hosted environment, inline cannot specify function \textit{main}.

\textbf{EXAMPLE - Avoid side-effect}
...
Assume \verb|a| assigned "1". Then \verb|min(++a,b);| makes \verb|a| equals 3, while \verb|MIN(++a,b);| makes \verb|a| equals 2.
