 ...
 * defined or a false conditional is being processed (%0, %1, %+1, %-1, %%foo
 *
...
static const enum pp_conds inverse_ccs[] = {
	c_NA, c_NAE, c_NB, c_NBE, c_NC, c_none, c_NE,c_none, c_NG, c_NGE, c_NL, c_NLE,
	c_A, c_AE, c_B, c_BE, c_C, c_E, c_G, c_GE, c_L, c_LE, c_O, c_P, c_S,
	c_Z, c_NO, c_NP, c_PO, c_PE, c_none, c_NS, c_NZ
};
...
_ESYM_C{
int StackSize = 4;
const char* StackPointer = "ebp";
int ArgOffset = 8;
int LocalOffset = 0;
evalfunc prep_evaluate;
#define evaluate prep_evaluate

int pass;                /* HACK: pass 0 = generate dependencies only */
StrList** dephead, ** deptail; /* Dependency list */
uint64_t unique;    /* unique identifier numbers */
Line* predef = NULL;
bool do_predef;
ListGen* list;
struct hash_table mmacros;// The current set of multi-line macros we have defined.
struct hash_table smacros;// The current set of single-line macros we have defined.
MMacro* defining;// The multi-line macro we are currently defining, or the %rep block we are currently reading, if any.
uint64_t nested_mac_count;
uint64_t nested_rep_count;
}
/*
...
#define PARAM_DELTA 16
  ...
  */
  //{TODO} static
_ESYM_C macros_t* stdmacpos = NULL;
 ...
 */
 //{TODO} static
_ESYM_C macros_t* extrastdmac = NULL;
_ESYM_C bool any_extrastdmac = NULL;

/*
 * Tokens are allocated in blocks to improve speed
 */
#define TOKEN_BLOCKSIZE 4096
_ESYM_C Token* freeTokens = NULL;//{TODO} static
struct Blocks {
	Blocks* next;
	void* chunk;
};
...
//{TODO} static
_ESYM_C Blocks blocks = { NULL, NULL };
...
#define tok_is_(x,v) (tok_type_((x), TOK_OTHER) && !StrCompare((x)->text,(v)))
#define tok_isnt_(x,v) ((x) && ((x)->type!=TOK_OTHER || StrCompare((x)->text,(v))))

//{TODO} static:
extern "C"     Context* cstk  = NULL;
extern "C"     Include* istk  = NULL;
extern "C"     IncPath* ipath = NULL;

static void verror(loglevel_t severity, const char* fmt, va_list arg)
{
	char buff[1024];
	vsnprintf(buff, sizeof(buff), fmt, arg);
	if (istk && istk->mstk && istk->mstk->name)
		aasm_log(severity, "(%s:%d) %s", istk->mstk->name, istk->mstk->lineno, buff);
	else
		aasm_log(severity, "%s", buff);
}
/*
 * Since preprocessor always operate only on the line that didn't
 * arrived yet, we should always use ERR_OFFBY1.
 */
static void error_common(loglevel_t severity, const char* fmt, ...)
{
	va_list arg;
...
void Preprocessor::include_path(char* path) {
	IncPath *i;

	i = malcof(IncPath);
	i->path = path ? StrHeap(path) : NULL;
	i->next = NULL;
...
void Preprocessor::pre_include(char* fname) {
...
void Preprocessor::pre_define(char* definition) {
	Token *def, *space;
	Line *l;
	char *equals;

	equals = (char*)StrIndexChar(definition, '=');
...
void Preprocessor::pre_undefine(char* definition) {
	Token* def, * space;
...
void Preprocessor::runtime(char* definition) {
	/*
	 * This function is used to assist with "runtime" preprocessor
	 * directives. (e.g. pp_runtime("%define __BITS__ 64");)
	 *
 ...
 */
static Token *new_Token(Token * next, enum pp_token_type type, const char *text, int txtlen)
{
	Token *t;
	int i;
	...
	if (type == TOK_WHITESPACE || text == NULL) {
		...
		if (txtlen == 0)
			txtlen = StrLength(text);
		t->text = (char*)malc(txtlen+1);
		MemCopyN(t->text, text, txtlen);
		t->text[txtlen] = '\0';
	}
	return t;
}
...
static Token *tokenize(char *line)
{
	...
	while (*line) {
		...
		if (*p == '%') {
			p++;
		if (*p == '+' && !ascii_isdigit(p[1])) {
		p++;
		type = TOK_PASTE;
		} else if (ascii_isdigit(*p) ||
			   ((*p == '-' || *p == '+') && ascii_isdigit(p[1]))) {
				do {
					p++;
				}
		...
		if (lvl) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "unterminated %[ construct");
		}
		type = TOK_INDIRECT;
		} else if (*p == '?') {
		type = TOK_PREPROC_Q; /* %? */
		...
		p = nasm_skip_string(p);
			...
			if (*p) {
				...
				//--auto clean flag
				log_pass1 = 1;
				aasm_log(_LOG_WARN, "unterminated string");
 ...
 */
static Context* get_ctx(const char* name, const char** namep, bool all_contexts);
static char *detoken(Token * tlist, bool expand_locals)
{
	Token *t;
	int len;
	...
	for (t = tlist; t; t = t->next) {
		...
		char *p;
			...
			if (ctx) {
				char buffer[40];
				snprintf(buffer, sizeof(buffer), "..@%" PRIu32 ".", ctx->number);//{} outsfmt series
				p = StrHeapAppend(buffer, q);
				memf(t->text);
				t->text = p;
			}
		}
		if (t->type == TOK_WHITESPACE) {
			...
			len += StrLength(t->text);
		}
	}
	p = line = (char*)malc(len + 1);
	for (t = tlist; t; t = t->next) {
		if (t->type == TOK_WHITESPACE) {
			*p++ = ' ';
		} else if (t->text) {
 ...
 * simple wrapper which calls either StrCompare or StrCompareInsensitive
 * depending on the value of the `casesense' parameter.
 */
static int mStrCompare(const char* p, const char* q, bool casesense)
{
	return (casesense ? StrCompare : StrCompareInsensitive)(p, q);
}
...
{
	return (casesense ? MemCompare: MemCompareInsensitive)(p, q, l);
}
...
static Context* get_ctx(const char* name, const char** namep, bool all_contexts)
{
	...
	if (!cstk) {
		//--auto clean flag
		aasm_log(_LOG_ERROR, "`%s': context stack is empty", name);
		return NULL;
	}
	...
	if (!ctx) {
		//--auto clean flag
		aasm_log(_LOG_ERROR, "`%s': context stack is only %d level%s deep", name, i, (i == 1 ? "" : "s"));
		return NULL;
	}
	...
	do {
		...
		while (m) {
			if (!mStrCompare(m->name, name, m->casesense))
				return ctx;
			m = m->next;
		}
		ctx = ctx->next;
 ...
 */
static void** hash_findi_add(struct hash_table* hash, const char* str)
{
	...
	strx = StrHeap(str);	/* Use a more efficient allocator here? */
...
static int ppscan(void* private_data, struct tokenval* tokval)
{
	 ...
	 * the token text.
	 */
	return tokval->t_type = (token_type)tline->text[0];
}
 ...
 */
static bool smacro_defined(Context* ctx, const char* name, int nparam, SMacro** defn, bool nocase);
...
static bool if_condition(Token* tline, enum preproc_token ct)
{
	...
	case PPC_IFCTX:
		...
		while (true) {
			...
			if (tline->type != TOK_ID) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%s' expects context identifiers", pp_directives[ct]);
				free_tlist(origline);
				return -1;
			}
			if (cstk && cstk->name && !StrCompareInsensitive(tline->text, cstk->name))
				j = true;
			tline = tline->next;
		}
		break;
		...
		while (tline) {
			...
			if (!tline || (tline->type != TOK_ID &&
				(tline->type != TOK_PREPROC_ID ||
					tline->text[1] != '$'))) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%s' expects macro identifiers", pp_directives[ct]);
				goto fail;
			}
	...
	case PPC_IFIDNI:
		...
		if (!tt) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%s' expects two comma-separated arguments", pp_directives[ct]);
			goto fail;
		}
		tt = tt->next;
		j = true;               /* assume equality unless proved not */
		while ((t->type != TOK_OTHER || StrCompare(t->text, ",")) && tt) {
			if (tt->type == TOK_OTHER && !StrCompare(tt->text, ",")) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%s': more than one comma on line", pp_directives[ct]);
				goto fail;
			}
			if (t->type == TOK_WHITESPACE) {
				t = t->next;
			...
			else if (mStrCompare(tt->text, t->text, i == PPC_IFIDN) != 0) {
				j = false;      /* found mismatching tokens */
				break;
			}
		...
		if ((t->type != TOK_OTHER || StrCompare(t->text, ",")) || tt)
	...
	case PPC_IFMACRO:
	{
		...
		if (!tok_type_(tline, TOK_ID)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%s' expects a macro name", pp_directives[ct]);
			goto fail;
		}
		searching.name = StrHeap(tline->text);
		...
		if (!tline) {
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%s' expects a parameter count or nothing", pp_directives[ct]);
		}
		else {
			...
			if (j) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "unable to parse parameter count `%s'", tline->text);
			}
		}
		if (tline && tok_is_(tline->next, "-")) {
			...
			if (tok_is_(tline, "*"))
				...
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%s' expects a parameter count after `-'", pp_directives[ct]);
			}
			else {
				...
				if (searching.nparam_min > searching.nparam_max) {
					//--auto clean flag
					aasm_log(_LOG_ERROR, "minimum parameter count exceeds maximum");
				}
			}
		}
		if (tline && tok_is_(tline->next, "+")) {
		...
		while (mmac) {
			if (!StrCompare(mmac->name, searching.name) &&
		...
		if (tline && tline->next) {
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "trailing garbage after %%ifmacro ignored");
		}
		memf(searching.name);
		j = found;
		break;
	...
	case PPC_IF:
		...
		if (tokval.t_type) {
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "trailing garbage after expression ignored");
		}
		if (!is_simple(evalresult)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "non-constant value given to `%s'", pp_directives[ct]);
			goto fail;
		}
	...
	default:
		//--auto clean flag
		aasm_log(_LOG_FATAL, "preprocessor directive `%s' not yet implemented", pp_directives[ct]);
		goto fail;
	}
 ...
 */
static bool define_smacro(Context* ctx, const char* mname, bool casesense, int nparam, Token* expansion)
{
	SMacro* smac, ** smhead;
	...
	if (smacro_defined(ctx, mname, nparam, &smac, casesense)) {
		if (!smac) {
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "single-line macro `%s' defined both with and without parameters", mname);
...
static void undef_smacro(Context* ctx, const char* mname)
{
	...
	if (smhead) {
		...
		sp = smhead;
		while ((s = *sp) != NULL) {
			if (!mStrCompare(s->name, mname, s->casesense)) {
				*sp = s->next;
				memf(s->name);
				free_tlist(s->expansion);
				memf(s);
...
static bool smacro_defined(Context* ctx, const char* name, int nparam, SMacro** defn, bool nocase)
{
	...
	while (m) {
		if (!mStrCompare(m->name, name, m->casesense && nocase) &&
 ...
 * _LOG_FATAL and bombing out completely if not. It should also try
 * the include path one by one until it finds the file or reaches
 * the end of the path.
 */
static bool in_list(const StrList* list, const char* str);
static FILE* inc_fopen(const char* file, StrList** dhead, StrList*** dtail, bool missing_ok)
{
	FILE* fp;
	char* prefix = (char*)"";
	IncPath* ip = ipath;
	int len = StrLength(file);
	size_t prefix_len = 0;
	StrList* sl;

	while (1) {
		sl = (StrList*)malc(prefix_len + len + 1 + sizeof(sl->next));
		MemCopyN(sl->str, prefix, prefix_len);
		MemCopyN(sl->str + prefix_len, file, len + 1);
		fp = fopen(sl->str, "r");
		...
		if (prefix) {
			prefix_len = StrLength(prefix);
		}
		else {
			/* -MG given and file not found */
			if (dhead && !in_list(*dhead, file)) {
				sl = (StrList*)malc(len + 1 + sizeof(sl->next));
				sl->next = NULL;
				StrCopy(sl->str, file);
				**dtail = sl;
				*dtail = &sl->next;
	...
	//--auto clean flag
	aasm_log(_LOG_FATAL, "unable to open include file `%s'", file);
...
static char* check_tasm_directive(char* line)
{
	...
	/* Skip whitespace */
	while (ascii_isspace(*p) && *p != 0)
		p++;

	/* Binary search for the directive name */
	i = -1;
	j = numsof(tasm_directives);
	len = 0;
	while (!ascii_isspace(p[len]) && p[len] != 0)
		len++;
	if (len) {
		oldchar = p[len];
		p[len] = 0;
		while (j - i > 1) {
			k = (j + i) / 2;
			m = StrCompareInsensitive(p, tasm_directives[k]);
			if (m == 0) {
...
_ESYM_C enum preproc_token pp_token_hash(const char* token);
static int do_directive(Token* tline)
{
	...
	case PP_INVALID:
		//--auto clean flag
		aasm_log(_LOG_ERROR, "unknown preprocessor directive `%s'", tline->text);
	...
	case PP_STACKSIZE:
		...
		if (!tline || tline->type != TOK_ID) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%stacksize' missing size parameter");
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		if (StrCompareInsensitive(tline->text, "flat") == 0) {
		...
		else if (StrCompareInsensitive(tline->text, "flat64") == 0) {
		...
		else if (StrCompareInsensitive(tline->text, "large") == 0) {
		...
		else if (StrCompareInsensitive(tline->text, "small") == 0) {
			/* All subsequent ARG directives are for a 16-bit stack,
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%stacksize' invalid size type");
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		free_tlist(origline);
	...
	case PP_ARG:
		...
		do {
			...
			if (!tline || tline->type != TOK_ID) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%%arg' missing argument parameter");
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
			arg = tline->text;
			...
			if (!tline || tline->type != TOK_OTHER
				|| tline->text[0] != ':') {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "Syntax error processing `%%arg' directive");
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
			tline = tline->next;
			if (!tline || tline->type != TOK_ID) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%%arg' missing size type parameter");
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
			...
			if (!size) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "Invalid size type for `%%arg' missing directive");
				free_tlist(tt);
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
	...
	case PP_LOCAL:
		...
		do {
			...
			if (!tline || tline->type != TOK_ID) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%%local' missing argument parameter");
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
			local = tline->text;
			...
			if (!tline || tline->type != TOK_OTHER
				|| tline->text[0] != ':') {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "Syntax error processing `%%local' directive");
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
			tline = tline->next;
			if (!tline || tline->type != TOK_ID) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%%local' missing size type parameter");
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
			...
			if (!size) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "Invalid size type for `%%local' missing directive");
				free_tlist(tt);
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
	...
	case PP_CLEAR:
		if (tline->next) {
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "trailing garbage after `%%clear' ignored");
		}
		free_macros();
		init_macros();
		free_tlist(origline);
	...
	case PP_DEPEND:
		...
		skip_white_(t);
		if (!t || (t->type != TOK_STRING && t->type != TOK_INTERNAL_STRING)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%depend' expects a file name");
			free_tlist(origline);
		...
		if (t->next) {
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "trailing garbage after `%%depend' ignored");
		}
		p = t->text;
	...
	case PP_INCLUDE:
		...
		if (!t || (t->type != TOK_STRING && t->type != TOK_INTERNAL_STRING)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%include' expects a file name");
			free_tlist(origline);
			return DIRECTIVE_FOUND;     /* but we did _something_ */
		}
		if (t->next) {
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "trailing garbage after `%%include' ignored");
		}
		p = t->text;
		if (t->type != TOK_INTERNAL_STRING)
			nasm_unquote(p, NULL);
		...
		if (!inc->fp) {
			...
			inc->fname = src_set_fname(StrHeap(p));
	...
	case PP_USE:
	{
		...
		if (!tline || (tline->type != TOK_STRING &&
			...
			tline->type != TOK_ID)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%use' expects a package name");
			free_tlist(origline);
			return DIRECTIVE_FOUND;     /* but we did _something_ */
		}
		if (tline->next) {
	...
	case PP_POP:
		...
		if (tline) {
			if (!tok_type_(tline, TOK_ID)) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%s' expects a context identifier", pp_directives[i]);
				free_tlist(origline);
				return DIRECTIVE_FOUND;     /* but we did _something_ */
			}
			if (tline->next) {
				//--auto clean flag
				log_pass1 = 1;
				aasm_log(_LOG_WARN, "trailing garbage after `%s' ignored", pp_directives[i]);
			}
			p = StrHeap(tline->text);
		}
		else {
			p = NULL;		/* Anonymous */
		}
		...
		if (i == PP_PUSH) {
			...
			if (!cstk) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%s': context stack is empty", pp_directives[i]);
			}
			else if (i == PP_POP) {
				if (p && (!cstk->name || StrCompareInsensitive(p, cstk->name))) {
					//--auto clean flag
					aasm_log(_LOG_ERROR, "`%%pop' in wrong context: %s, expected %s", cstk->name ? cstk->name : "anonymous", p);
				}
				else ctx_pop();
			}
			else {
	...
	case PP_WARNING:
		//--auto clean flag
		severity = _LOG_WARN; log_warnt = ERR_WARN_USER;
		goto issue_error;
	...
	CASE_PP_ELIF:
		if (!istk->conds) {
			//--auto clean flag
			aasm_log(_LOG_FATAL, "`%s': no matching `%%if'", pp_directives[i]);
		}
		...
		case COND_ELSE_FALSE:
			//--auto clean flag
			log_pass1 = 1;
			error_precond(_LOG_WARN, "`%%elif' after `%%else' ignored");
	...
	case PP_ELSE:
		if (tline->next) {
			//--auto clean flag
			log_pass1 = 1;
			error_precond(_LOG_WARN, "trailing garbage after `%%else' ignored");
		}
		if (!istk->conds) {
			//--auto clean flag
			aasm_log(_LOG_FATAL, "`%%else': no matching `%%if'");
		}
		switch (istk->conds->state) {
		case COND_IF_TRUE:
		case COND_DONE:
		...
		case COND_ELSE_FALSE:
			//--auto clean flag
			log_pass1 = 1;
			error_precond(_LOG_WARN, "`%%else' after `%%else' ignored.");
			istk->conds->state = COND_NEVER;
			break;
		}
		free_tlist(origline);
	...
	case PP_ENDIF:
		if (tline->next) {
			//--auto clean flag
			log_pass1 = 1;
			error_precond(_LOG_WARN, "trailing garbage after `%%endif' ignored");
		}
		if (!istk->conds) {
			//--auto clean flag
			aasm_log(_LOG_FATAL, "`%%endif': no matching `%%if'");
		}
		cond = istk->conds;
	...
	case PP_IMACRO:
		if (defining) {
			//--auto clean flag
			aasm_log(_LOG_FATAL, "`%%%smacro': already defining a macro", (i == PP_IMACRO ? "i" : ""));
			return DIRECTIVE_FOUND;
		}
		defining = malcof(MMacro);
		...
		while (mmac) {
			if (!StrCompare(mmac->name, defining->name) &&
				...
				//--auto clean flag
				log_pass1 = 1;
				aasm_log(_LOG_WARN, "redefining multi-line macro `%s'", defining->name);
				return DIRECTIVE_FOUND;
			}
			mmac = mmac->next;
		}
	...
	case PP_ENDMACRO:
		if (!(defining && defining->name)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%s': not defining a macro", tline->text);
			return DIRECTIVE_FOUND;
		}
	...
	case PP_UNIMACRO:
	{
		...
		while (mmac_p && *mmac_p) {
			...
			if (mmac->casesense == spec.casesense &&
				!mStrCompare(mmac->name, spec.name, spec.casesense) &&
	...
	case PP_ROTATE:
		...
		if (tline->next == NULL) {
			free_tlist(origline);
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%rotate' missing rotate count");
			return DIRECTIVE_FOUND;
		}
		...
		if (!is_simple(evalresult)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "non-constant value given to `%%rotate'");
			return DIRECTIVE_FOUND;
		}
		mmac = istk->mstk;
		...
		if (!mmac) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%rotate' invoked outside a macro call");
		}
		else if (mmac->nparam == 0) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%rotate' invoked within macro without parameters");
		}
		else {
	...
	case PP_REP:
		...
		if (tok_type_(tline, TOK_ID) &&
			StrCompareInsensitive(tline->text, ".nolist") == 0) {
			nolist = true;
			do {
				tline = tline->next;
			} while (tok_type_(tline, TOK_WHITESPACE));
		...
		if (tline) {
			...
			if (!is_simple(evalresult)) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "non-constant value given to `%%rep'");
				return DIRECTIVE_FOUND;
			}
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%rep' expects a repeat count");
			count = 0;
		}
		free_tlist(origline);
	...
	case PP_ENDREP:
		if (!defining || defining->name) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%endrep': no matching `%%rep'");
			return DIRECTIVE_FOUND;
		}
	...
	case PP_EXITREP:
		...
		if (l)
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%exitrep' not within `%%rep' block");
		}
		free_tlist(origline);
		return DIRECTIVE_FOUND;
	...
	case PP_IDEFINE:
		...
		if (!tline || (tline->type != TOK_ID &&
			(tline->type != TOK_PREPROC_ID ||
				tline->text[1] != '$'))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%s' expects a macro identifier", pp_directives[i]);
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		...
		if (tok_is_(tline, "(")) {
			...
			while (1) {
				...
				if (!tline) {
					//--auto clean flag
					aasm_log(_LOG_ERROR, "parameter identifier expected");
					free_tlist(origline);
					return DIRECTIVE_FOUND;
				}
				if (tline->type != TOK_ID) {
					//--auto clean flag
					aasm_log(_LOG_ERROR, "`%s': parameter identifier expected", tline->text);
					free_tlist(origline);
					return DIRECTIVE_FOUND;
				}
				tline->type = pp_token_type(TOK_SMAC_PARAM + nparam++);
				tline = tline->next;
				skip_white_(tline);
				if (tok_is_(tline, ",")) {
					tline = tline->next;
					...
					if (!tok_is_(tline, ")")) {
						//--auto clean flag
						aasm_log(_LOG_ERROR, "`)' expected to terminate macro template");
						free_tlist(origline);
						return DIRECTIVE_FOUND;
					}
					break;
	...
	case PP_UNDEF:
		...
		if (!tline || (tline->type != TOK_ID &&
			(tline->type != TOK_PREPROC_ID ||
				tline->text[1] != '$'))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%undef' expects a macro identifier");
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		if (tline->next) {
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "trailing garbage after macro name ignored");
		}
	...
	case PP_PATHSEARCH:
	{
		...
		if (!tline || (tline->type != TOK_ID &&
			(tline->type != TOK_PREPROC_ID ||
				tline->text[1] != '$'))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%pathsearch' expects a macro identifier as first parameter");
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		ctx = get_ctx(tline->text, &mname, false);
		...
		if (!t || (t->type != TOK_STRING &&
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%pathsearch' expects a file name");
			free_tlist(tline);
			free_tlist(origline);
			return DIRECTIVE_FOUND;     /* but we did _something_ */
		}
		if (t->next) {
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "trailing garbage after `%%pathsearch' ignored");
		}
		p = t->text;
		if (t->type != TOK_INTERNAL_STRING)
			nasm_unquote(p, NULL);
	...
	case PP_STRLEN:
		...
		if (!tline || (tline->type != TOK_ID &&
			(tline->type != TOK_PREPROC_ID ||
				tline->text[1] != '$'))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%StrLength' expects a macro identifier as first parameter");
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		ctx = get_ctx(tline->text, &mname, false);
		...
		if (t->type != TOK_STRING) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%StrLength` requires string as second parameter");
			free_tlist(tline);
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
	...
	case PP_STRCAT:
		...
		if (!tline || (tline->type != TOK_ID &&
			(tline->type != TOK_PREPROC_ID ||
				tline->text[1] != '$'))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%strcat' expects a macro identifier as first parameter");
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		ctx = get_ctx(tline->text, &mname, false);
		...
		for (t = tline; t; t = t->next) {
			...
			default:
				//--auto clean flag
				aasm_log(_LOG_ERROR, "non-string passed to `%%strcat' (%d)", t->type);
				free_tlist(tline);
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
	...
	case PP_SUBSTR:
	{
		...
		if (!tline || (tline->type != TOK_ID &&
			(tline->type != TOK_PREPROC_ID ||
				tline->text[1] != '$'))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%substr' expects a macro identifier as first parameter");
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		ctx = get_ctx(tline->text, &mname, false);
		...
		if (t->type != TOK_STRING) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%substr` requires string as second parameter");
			free_tlist(tline);
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		...
		if (!evalresult) {
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "non-constant value given to `%%substr`");
			free_tlist(tline);
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
	...
	case PP_IASSIGN:
		...
		if (!tline || (tline->type != TOK_ID &&
			(tline->type != TOK_PREPROC_ID ||
				tline->text[1] != '$'))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%%sassign' expects a macro identifier", (i == PP_IASSIGN ? "i" : ""));
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		ctx = get_ctx(tline->text, &mname, false);
		...
		if (!is_simple(evalresult)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "non-constant value given to `%%%sassign'", (i == PP_IASSIGN ? "i" : ""));
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
	...
	case PP_LINE:
		...
		if (!tok_type_(tline, TOK_NUMBER)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%%line' expects line number");
			free_tlist(origline);
			return DIRECTIVE_FOUND;
		}
		...
		if (tok_is_(tline, "+")) {
			...
			if (!tok_type_(tline, TOK_NUMBER)) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%%line' expects line increment");
				free_tlist(origline);
				return DIRECTIVE_FOUND;
			}
		...
		istk->lineinc = m;
		if (tline) {
			memf(src_set_fname(detoken(tline, false)));
		}
		free_tlist(origline);
		return DIRECTIVE_FOUND;
	...
	default:
		//--auto clean flag
		aasm_log(_LOG_FATAL, "preprocessor directive `%s' not yet implemented", pp_directives[i]);
		return DIRECTIVE_FOUND;
	}
}
...
static int find_cc(Token* t)
{
	...
	tt = t->next;
	skip_white_(tt);
	if (tt && (tt->type != TOK_OTHER || StrCompare(tt->text, ",")))
		return -1;
	...
	while (j - i > 1) {
		k = (j + i) / 2;
		m = StrCompareInsensitive(t->text, conditions[k]);
		if (m == 0) {
			i = k;
			j = -2;
			break;
	...
	while ((t = *tail) && (tt = t->next)) {
		...
		case TOK_FLOAT:
		{
			...
			if (tt != t->next) {
				...
				len += StrLength(t->text);
				p = tmp = (char*)malc(len + 1);

				while (t != tt) {
					StrCopy(p, t->text);
					p = (char*)StrIndexChar(p, '\0');
					t = delete_Token(t);
				}
		...
		case TOK_PASTE:		/* %+ */
			if (handle_paste_tokens) {
				...
				if (tt) {
					tmp = StrHeapAppend(t->text, tt->text);
					delete_Token(t);
					tt = delete_Token(tt);
					t = *tail = tokenize(tmp);
					memf(tmp);
...
static MMacro* is_mmacro(Token* tline, Token*** params_array)
{
	...
	for (m = head; m; m = m->next)
		if (!mStrCompare(m->name, tline->text, m->casesense))
			break;
	if (!m)
		return NULL;
	...
	while (m) {
		if (m->nparam_min <= nparam
...
#if 0
				//--auto clean flag
				aasm_log(_LOG_ERROR, "self-reference in multi-line macro `%s'", m->name);
#endif
				memf(params);
				return NULL;
			}
			...
			if (!params) {      /* need this special case */
				params = (Token**)malc(sizeof(*params));
				nparam = 0;
			}
			params[nparam] = NULL;
			*params_array = params;
	 ...
	 */
	 //--auto clean flag
	log_pass1 = 1;
	log_warnt = ERR_WARN_MNP;
	aasm_log(_LOG_WARN, "macro `%s' exists, but not taking %d parameters", tline->text, nparam);
	memf(params);
	return NULL;
}
	...
	while (t) {
		...
		if (t) {                /* got a comma/brace */
			...
			if (brace) {
				...
				if (tok_isnt_(t, ",")) {
					//--auto clean flag
					aasm_log(_LOG_ERROR, "braces do not enclose all of macro parameter");
					while (tok_isnt_(t, ","))
						t = t->next;
				}
				if (t)
...
static Token* expand_mmac_params(Token* tline)
{
	...
	while (tline) {
		if (tline->type == TOK_PREPROC_ID &&
			...
			if (!mac) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%s': not in a macro call", t->text);
			}
			else
				switch (t->text[1]) {
					/*
				...
				case '%':
					...
					snprintf(tmpbuf, sizeof(tmpbuf), "..@%" PRIu64 ".",
						mac->unique);
					text = StrHeapAppend(tmpbuf, t->text + 2);
					break;
				case '-':
					n = atoins(t->text + 2) - 1;
					if (n >= mac->nparam)
					...
					if (cc == -1) {
						//--auto clean flag
						aasm_log(_LOG_ERROR, "macro parameter %d is not a condition code", n + 1);
						text = NULL;
					}
					else {
						type = TOK_ID;
						if (inverse_ccs[cc] == -1) {
							//--auto clean flag
							aasm_log(_LOG_ERROR, "condition code `%s' is not invertible", conditions[cc]);
							text = NULL;
						}
						else
							text = StrHeap(conditions[inverse_ccs[cc]]);
					}
					break;
				case '+':
					n = atoins(t->text + 2) - 1;
 ...
 */
#define DEADMAN_LIMIT _IMM1S(20)
...
again:
	...
	while (tline) {             /* main token loop */
		if (!--deadman) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "interminable macro recursion");
			break;
		}
		...
		if ((mname = tline->text)) {
			 ...
			 */
			for (m = head; m; m = m->next)
				if (!mStrCompare(m->name, mname, m->casesense))
					break;
			if (m) {
				mstart = tline;
				params = NULL;
				...
				if (m->nparam == 0) {
					...
					if (!m->expansion) {
						...
						if (!StrCompare("__LINE__", m->name)) {
							memf(tline->text);
							make_tok_num(tline, src_get_linnum());
							continue;
						}
						if (!StrCompare("__BITS__", m->name)) {
							memf(tline->text);
					...
					if (!tok_is_(tline, "(")) {
						...
						nparam = 0;
						sparam = PARAM_DELTA;
						params = (Token**)malc(sparam * sizeof(Token*));
						params[0] = tline->next;
						paramsize = (int*)malc(sparam * sizeof(int));
						paramsize[0] = 0;
						while (true) {  /* parameter loop */
							...
							if (!tline) {
								//--auto clean flag
								aasm_log(_LOG_ERROR, "macro call expects terminating `)'");
								break;
							}
						...
						if (!m) {
							...
							log_pass1 = 1;
							log_warnt = ERR_WARN_MNP;
							aasm_log(_LOG_WARN, "macro `%s' exists, but not taking %d parameters", mstart->text, nparam);
						}
					}
				}
				if (m && m->in_progress)
...
static bool parse_mmacro_spec(Token* tline, MMacro* def, const char* directive)
{
	...
	if (!tok_type_(tline, TOK_ID)) {
		//--auto clean flag
		aasm_log(_LOG_ERROR, "`%s' expects a macro name", directive);
		return false;
	}

	def->name = StrHeap(tline->text);
	def->plus = false;
	def->nolist = false;
	def->in_progress = 0;
	def->rep_nest = NULL;
	...
	if (!tok_type_(tline, TOK_NUMBER)) {
		//--auto clean flag
		aasm_log(_LOG_ERROR, "`%s' expects a parameter count", directive);
	}
	else {
	...
	if (tline && tok_is_(tline->next, "-")) {
		...
		if (tok_is_(tline, "*")) {
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`%s' expects a parameter count after `-'", directive);
		}
		else {
			def->nparam_max = readnum(tline->text, &err);
			if (err) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "unable to parse parameter count `%s'",
					tline->text);
			}
	...
	if (tline && tok_type_(tline->next, TOK_ID) &&
		!StrCompareInsensitive(tline->next->text, ".nolist")) {
		tline = tline->next;
		def->nolist = true;
	}
	...
	if (def->defaults &&
		...
		//--auto clean flag
		log_pass1 = 1;
		log_warnt = ERR_WARN_MDP;
		aasm_log(_LOG_WARN, "too many default macro parameters");
	}
...
static int parse_size(const char* str) {
	...
	return sizes[bsii(str, size_names, numsof(size_names)) + 1];
}

// ---- ---- TODEL ---- ---- //

_ESYM_C void pp_include_path(char*) {
	Preprocessor::include_path(NULL);
}

// ---- ---- STATIC ---- ---- //

static void
pp_reset(char* file, int apass, evalfunc eval, ListGen* listgen, StrList** deplist)
{
	Token* t;
	...
	if (!istk->fp) {
		//--auto clean flag
		log_0file = 1;
		aasm_log(_LOG_FATAL, "unable to open input file `%s'", file);
	}
	defining = NULL;
	nested_mac_count = 0;
	nested_rep_count = 0;
...
static char* prepreproc(char* line)
{
	...
	if (line[0] == '#' && line[1] == ' ') {
		...
		fname = oldline + 2;
		lineno = atoins(fname);
		fname += StrSpanInclude(fname, "0123456789 ");
		if (*fname == '"')
			fname++;
		fnlen = StrSpanExclude(fname, "\"");
		line = (char*)malc(20 + fnlen);
...
static char* read_line(void)
{
	...
	if (stdmacpos) {
		...
		while ((c = *stdmacpos++)) {
			if (c >= 0x80) {
				MemCopyN(q, pp_directives[c - 0x80], pp_directives_len[c - 0x80]);
	 ...
	 */
	buffer[StrSpanExclude(buffer, "\032")] = '\0';
...
static int expand_mmacro(Token* tline)
{
	...
	paramlen = nparam ? (int*)malc(nparam * sizeof(*paramlen)) : NULL;
	...
	while (1) {
		...
		while (istk->expansion && istk->expansion->finishes) {
			...
			if (!l->finishes->name && l->finishes->in_progress > 1) {
				...
				if (defining) {
					if (defining->name) {
						//--auto clean flag
						aasm_log(_LOG_PANIC, "defining with name in expansion");
					}
					else if (istk->mstk->name) {
						//--auto clean flag
						aasm_log(_LOG_FATAL, "`%%rep' without `%%endrep' within expansion of macro `%s'", istk->mstk->name);
					}
				}
		...
		while (1) {             /* until we get a line we can use */
			 ...
			 */
			{
				...
				if (i->conds) {
					//--auto clean flag
					aasm_log(_LOG_FATAL, "expected `%%endif' before end of file");
				}
				/* only set line and file name if there's a next node */
				if (i->next) {
					src_set_linnum(i->lineno);
					memf(src_set_fname(i->fname));
				}
				istk = i->next;
...
static void pp_cleanup(int pass)
{
	if (defining) {
		//--auto clean flag
		if (defining->name) {
			aasm_log(_LOG_ERROR, "end of file while still defining macro `%s'", defining->name);
		}
		else {
			aasm_log(_LOG_ERROR, "end of file while still in %%rep");
		}
