// ASCII C TAB4 CRLF
// Docutitle: (Module) code generation
...
static void assert_no_prefix(insn * ins, enum prefix_pos pos)
{
	if (ins->prefixes[pos]) {
		//--auto clean flag
		aasm_log(_LOG_ERROR, "invalid %s prefix", prefix_name(ins->prefixes[pos]));
	}
}
...
static void warn_overflow(int size, const struct operand *o)
{
	if (size < 8 && o->wrt == NO_SEG && o->segment == NO_SEG) {
		...
		if (data < ~lim || data > lim) {
			...
			aasm_log(_LOG_WARN, "%s data exceeds bounds", size_name(size));
		}
	}
}
/*
		...
		int32_t segment, int32_t wrt)
{
	...
	if (size > 8) {
		//--auto clean flag
		aasm_log(_LOG_PANIC, "OUT_ADDRESS with size > 8");
		return;
	}
...
int64_t assemble(int32_t segment, int64_t offset, int bits, uint32_t cp,
	insn* instruction, struct outffmt* output,
	ListGen* listgen)
{
	...
	if (wsize) {
		...
		if (t < 0) {
			//--auto clean flag
			aasm_log(_LOG_PANIC, "instruction->times < 0 (%ld) in assemble()", t);
		}
			...
			for (e = instruction->eops; e; e = e->next) {
				if (e->type == EOT_DB_NUMBER) {
					if (wsize == 1) {
						if (e->segment != NO_SEG) {
							//--auto clean flag
							aasm_log(_LOG_ERROR, "one-byte relocation attempted");
						}
						else {
						...
						//--auto clean flag
						aasm_log(_LOG_ERROR, "integer supplied to a DT, DO or DY instruction");
					}
					else
	...
	if (instruction->opcode == I_INCBIN) {
		...
		if (!fp) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`incbin': unable to open file `%s'", fname);
		}
		else if (fseek(fp, 0L, SEEK_END) < 0) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`incbin': unable to seek on file `%s'", fname);
		}
		else {
			...
			while (t--) {
				...
				while (l > 0) {
					...
					if (!m) {
						 ...
						 */
						//--auto clean flag
						aasm_log(_LOG_ERROR, "`incbin': unexpected EOF while reading file `%s'", fname);
	...
	for (temp = nasm_instructions[instruction->opcode]; temp->opcode != -1; temp++) {
	  ...
	  /* Matches! */
			...
			{
				//--auto clean flag
				aasm_log(_LOG_PANIC, "errors made it through from pass one");
			}
			else
				while (itimes--) {
					for (j = 0; j < MAXPREFIX; j++) {
						...
						case R_CS:
							if (bits == 64) {
								//--auto clean flag
								log_pass2 = 1;
								aasm_log(_LOG_WARN, "cs segment base generated, but will be ignored in 64-bit mode");
							}
							c = 0x2E;
							break;
						case R_DS:
							if (bits == 64) {
								//--auto clean flag
								log_pass2 = 1;
								aasm_log(_LOG_WARN, "ds segment base generated, but will be ignored in 64-bit mode");
							}
							c = 0x3E;
							break;
						case R_ES:
							if (bits == 64) {
								//--auto clean flag
								log_pass2 = 1;
								aasm_log(_LOG_WARN, "es segment base generated, but will be ignored in 64-bit mode");
							}
							c = 0x26;
							break;
						case R_FS:
						...
						case R_SS:
							if (bits == 64) {
								//--auto clean flag
								log_pass2 = 1;
								aasm_log(_LOG_WARN, "ss segment base generated, but will be ignored in 64-bit mode");
							}
							c = 0x36;
							break;
						case R_SEGR6:
						case R_SEGR7:
							//--auto clean flag
							aasm_log(_LOG_ERROR, "segr6 and segr7 cannot be used as prefixes");
							break;
						case P_A16:
							if (bits == 64) {
								//--auto clean flag
								aasm_log(_LOG_ERROR, "16-bit addressing is not supported in 64-bit mode");
							}
							else if (bits != 16)
								c = 0x67;
							break;
						...
						case P_A64:
							if (bits != 64) {
								//--auto clean flag
								aasm_log(_LOG_ERROR, "64-bit addressing is only supported in 64-bit mode");
							}
							break;
						case P_ASP:
							c = 0x67;
						...
						default:
							//--auto clean flag
							aasm_log(_LOG_PANIC, "invalid instruction prefix");
						}
						if (c != 0) {
	...
	if (temp->opcode == -1) {   /* didn't match any instruction */
		...
		switch (size_prob) {
		case 1:
			aasm_log(_LOG_ERROR, "operation size not specified");
			break;
		case 2:
			aasm_log(_LOG_ERROR, "mismatch in operand sizes");
			break;
		case 3:
			aasm_log(_LOG_ERROR, "no instruction for this cpu level");
			break;
		case 4:
			aasm_log(_LOG_ERROR, "instruction not supported in %d-bit mode", bits);
			break;
		default:
			aasm_log(_LOG_ERROR, "invalid combination of opcode and operands");
			break;
		}
	}
	return 0;
	...
	insn* ins, const uint8_t* codes)
{
	...
	while (*codes) {
		...
		case 0340:
			if (ins->oprs[0].segment != NO_SEG) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "attempt to reserve non-constant quantity of BSS space");
			}
			else
		...
		case 0373:
				...
				case4(0234) :
			{
				...
				if (!process_ea(opy, &ea_data, bits,
					...
					//--auto clean flag
					aasm_log(_LOG_ERROR, "invalid effective address");
					return -1;
				}
				else {
					ins->rex |= ea_data.rex;
		...
		default:
			//--auto clean flag
			aasm_log(_LOG_PANIC, "internal instruction table corrupt : instruction code \\%o (0x%02X) given", c, c);
			break;
		}
	}
	...
	if (ins->rex & REX_NH) {
		if (ins->rex & REX_H) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "instruction cannot use high registers");
			return -1;
		}
	...
	if (ins->rex & REX_V) {
		...
		if (ins->rex & REX_H) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "cannot use high register in vex instruction");
			return -1;
		}
		...
		if (bits != 64 && ((ins->rex & bad32) || ins->drexdst > 7)) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "invalid operands in non-64-bit mode");
			return -1;
		}
		...
		if (ins->rex & REX_H) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "cannot use high register in drex instruction");
			return -1;
		}
		...
		if (ins->rex & REX_H) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "cannot use high register in rex instruction");
			return -1;
		}
		else if (bits == 64) {
			length++;
	...
	int64_t insn_end)
{
	...
	const uint8_t* codes = temp->code;
	uint8_t opex = 0;
	if (ins->opcode == I_ARINA) {
		out(offset, segment, codes, OUT_RAWDATA, 2, NO_SEG, NO_SEG);
	}
	else while (*codes) {
		c = *codes++;
		op1 = (c & 3) + ((opex & 1) << 2);
		...
		case4(014) :
			/* The test for BITS8 and SBYTE here is intended to avoid warning on optimizer actions due to SBYTE, while still warn on explicit BYTE directives.  Also warn, obviously, if the optimizer isn't enabled. */
			if (((opx->type & BITS8) ||
				...
				log_pass2 = 1;
				log_warnt = ERR_WARN_NOV;
				aasm_log(_LOG_WARN, "signed byte value exceeds bounds");
			}
		...
		case4(020) :
			if (opx->offset < -256 || opx->offset > 255) {
				...
				log_pass2 = 1;
				log_warnt = ERR_WARN_NOV;
				aasm_log(_LOG_WARN, "byte value exceeds bounds");
			}
			if (opx->segment != NO_SEG) {
				data = opx->offset;
				out(offset, segment, &data, OUT_ADDRESS, 1,
		...
		case4(024) :
			if (opx->offset < 0 || opx->offset > 255) {
				...
				log_pass2 = 1;
				log_warnt = ERR_WARN_NOV;
				aasm_log(_LOG_WARN, "unsigned byte value exceeds bounds");
			}
			if (opx->segment != NO_SEG) {
				data = opx->offset;
				out(offset, segment, &data, OUT_ADDRESS, 1,
		...
		case4(050) :
			if (opx->segment != segment) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "short relative jump outside segment");
			}
			data = opx->offset - insn_end;
			if (data > 127 || data < -128)
			{
				//--auto clean flag
				aasm_log(_LOG_ERROR, "short jump is out of range");
			}
			bytes[0] = data;
			out(offset, segment, bytes, OUT_RAWDATA, 1, NO_SEG, NO_SEG);
			offset += 1;
		...
		case4(074) :
			if (opx->segment == NO_SEG) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "value referenced by FAR is not relocatable");
			}
			data = 0;
			out(offset, segment, &data, OUT_ADDRESS, 2,
		...
		case 0172:
			...
			if (opx->segment != NO_SEG || opx->wrt != NO_SEG) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "non-absolute expression not permitted as argument %d", c & 7);
			}
			else {
				if (opx->offset & ~15) {
					//--auto clean flag
					log_pass2 = 1;
					log_warnt = ERR_WARN_NOV;
					aasm_log(_LOG_WARN, "four-bit argument exceeds bounds");
				}
		...
		case4(0250) :
			...
			if (opx->wrt == NO_SEG && opx->segment == NO_SEG &&
				...
				log_pass2 = 1;
				log_warnt = ERR_WARN_NOV;
				aasm_log(_LOG_WARN, "signed dword immediate exceeds bounds");
			}
			if (is_sbyte32(opx)) {
				bytes[0] = data;
				out(offset, segment, bytes, OUT_RAWDATA, 1, NO_SEG,
		...
		case 0340:
			if (ins->oprs[0].segment != NO_SEG) {
				//--auto clean flag
				aasm_log(_LOG_PANIC, "non-constant BSS size in pass two");
			}
			else {
		...
		case 0345:
			...
			default:
				//--auto clean flag
				aasm_log(_LOG_PANIC, "bizarre 8086 segment register received");
			}
			out(offset, segment, bytes, OUT_RAWDATA, 1, NO_SEG, NO_SEG);
			offset++;
			break;
		...
		case 0347:
			...
			default:
				//--auto clean flag
				aasm_log(_LOG_PANIC, "bizarre 386 segment register received");
			}
			out(offset, segment, bytes, OUT_RAWDATA, 1, NO_SEG, NO_SEG);
			offset++;
			break;
		...
		/* DREX suffixes come between the SIB and the displacement */
				...
				default:
					/* Impossible! */
					//--auto clean flag
					aasm_log(_LOG_PANIC, "Invalid amount of bytes (%d) for offset?!", ea_data.bytes);
					break;
				}
				offset += s;
			}
...
static void add_asp(insn* ins, int addrbits)
{
 ...
 /* Add an address size prefix */
		...
		/* Impossible... */
		//--auto clean flag
		aasm_log(_LOG_ERROR, "impossible combination of address sizes");
