// ASCII C TAB4 CRLF
// Docutitle: (Format.Executable) [outdated] Unix AOUT
// Codifiers: @ArinaMgk
...
static void aoutg_init(FILE * fp, ldfunc ldef,
					   evalfunc eval)
{
	aoutfp = fp;
	evaluate = eval;
	(void)ldef;                 /* placate optimisers */
	stext.data = StrbuffNew(1L);
	stext.head = NULL;
	stext.tail = &stext.head;
	sdata.data = StrbuffNew(1L);
	sdata.head = NULL;
	...
	stext.index = SegAlloc();
	sdata.index = SegAlloc();
	sbss.index = SegAlloc();
...
static void aout_init(FILE * fp, ldfunc ldef, evalfunc eval)
{
	bsd = false;
	aoutg_init(fp, ldef, eval);
...
extern struct outffmt of_aoutb;

static void aoutb_init(FILE * fp, ldfunc ldef,
					   evalfunc eval)
{
	bsd = true;
	aoutg_init(fp, ldef, eval);
...
static void aout_cleanup(int debuginfo)
{
	...
	aout_write();
	fclose(aoutfp);
	StrbuffFree(stext.data);
	while (stext.head) {
		r = stext.head;
		stext.head = stext.head->next;
		memf(r);
	}
	StrbuffFree(sdata.data);
	while (sdata.head) {
		r = sdata.head;
...
static int32_t aout_section_names(rostr name, int pass, int *bits)
{
	...
	if (is_global && sym->type != SYM_GLOBAL) {
		...
		if (special) {
			...
			if (special[n]) {
				...
				if (!bsd) {
					aasm_log(_LOG_ERROR, "Linux a.out does not support"
								  ...
								  int32_t offset, int bytes)
{
	...
	if (segment == stext.index)
		...
		asym = sbss.asym;
	if (!asym)
		aasm_log(_LOG_ERROR, "`..gotoff' relocations require a non-global"
					 ...
					 int32_t segment, int32_t wrt)
{
	...
	if (type == OUT_RESERVE) {
		...
		aout_sect_write(s, (uint8*)data, size);
	} else if (type == OUT_ADDRESS) {
		addr = *(int64_t *)data;
		if (segment != NO_SEG) {
			if (segment % 2) {
				aasm_log(_LOG_ERROR, "a.out format does not support"
		...
		if (segment != NO_SEG && segment % 2) {
			...
			if (wrt == NO_SEG) {
				...
				aasm_log(_LOG_ERROR, "Linux a.out format does not support"
					  " any use of WRT");
				wrt = NO_SEG;   /* we can at least _try_ to continue */
			} else if (wrt == aout_plt_sect + 1) {
				is_pic = 0x40;
				aout_add_reloc(s, segment, RELTYPE_PLT, 2);
					   ...
					   wrt == aout_got_sect + 1) {
				aasm_log(_LOG_ERROR, "a.out format cannot produce PC-"
					  "relative GOT references");
			} else {
				aasm_log(_LOG_ERROR, "a.out format does not support this"
					  " use of WRT");
				wrt = NO_SEG;   /* we can at least _try_ to continue */
			}
		}
...
static void aout_fixup_relocs(struct Section *sect)
{
	...
	StrbuffRewind(sect->data);
	for (r = sect->head; r; r = r->next) {
		uint8_t *p, *q, blk[4];
		int32_t l;

		StrbuffPopx(sect->data, r->address, blk, (int32_t)r->bytes);
		p = q = blk;
		l = *p++;
		if (r->bytes > 1) {
			l += ((int32_t)*p++) << 8;
		...
		if (r->bytes == 4)
			...
			*q++ = l & 0xFF;
		StrbuffSendx(sect->data, r->address, blk, (int32_t)r->bytes);
	}
}
...
static void aout_write(void)
{
	 ...
	 */
	StrbuffSendFile(stext.data, aoutfp);
	StrbuffSendFile(sdata.data, aoutfp);
