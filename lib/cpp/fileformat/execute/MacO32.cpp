// ASCII C TAB4 CRLF
// Docutitle: (Format.Executable) NeXTstep/OpenStep/Rhapsody/Darwin/MacOS X (x86) object files
// Codifiers: @ArinaMgk
...
// Most of this file is, like Mach-O itself, based on a.out. For more guidelines see outaout.c.
...
static void debug_section_relocs(struct section*);// unused
						 ...
						 int32_t section, int32_t wrt)
{
	...
	if (wrt != NO_SEG) {
		wrt = NO_SEG;
		aasm_log(_LOG_ERROR, "WRT not supported by Mach-O output format");
...
static int32_t macho_section(rostr name, int pass, int *bits)
{
	...
	for (sm = sectmap; sm->nasmsect != NULL; ++sm) {
		...
		if (!StrCompare(name, sm->nasmsect)) {
			...
			if (index == -1) {
				s = *sectstail = malcof(struct section);
				s->next = NULL;
				sectstail = &s->next;
...
static void macho_write_section (void)
{
	...
	for (r = s->relocs; r != NULL; r = r->next) {
		StrbuffPopx(s->data, r->addr, blk, (int32_t)r->length << 1);
		p = q = blk;
		l = *p++;
		...
		*q++ = l & 0xFF;
		...
		StrbuffSendx(s->data, r->addr, blk, (int32_t)r->length << 1);
	}
