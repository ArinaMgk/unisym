// ASCII C TAB4 CRLF
// Docutitle: (Module) Basic cp Utility using POSIX API and consio
...
static int copy_file(const char* src, const char* dest) {
	...
	// Open destination for writing, create if not exist, truncate if exist
	...
	// Loop to copy file contents block-by-block
...
static int copy_recursive(const char* src, const char* dest) {
	...
	if (S_ISDIR(st.st_mode)) {
		...
		while ((entry = readdir(dir)) != NULL) {
			...
			// Construct destination child path
			char sub_dest[512];
			stduint len_dest = 0;
...
int main(int argc, char* argv[]) {
		...
		if (argv[i][0] == '-') {
			for (int j = 1; argv[i][j] != '\0'; j++) {
				if (argv[i][j] == 'r' || argv[i][j] == 'R') {
					...
					outsfmt("cp: invalid option -- '%c'\n\r", argv[i][j]);
					return 1;
				}
			}
			if (file_args_start == i) {
				file_args_start = i + 1;
			}
		}
	}

	if (argc - file_args_start < 2) {
		outsfmt("cp: missing file operand\n\r");
		return 1;
	}

	const char* src = argv[file_args_start];
	const char* dest = argv[file_args_start + 1];

	struct stat st_src;
	if (stat(src, &st_src) < 0) {
		outsfmt("cp: cannot stat '%s': No such file or directory\n\r", src);
		return 1;
	}

	if (S_ISDIR(st_src.st_mode) && !opt_recursive) {
		outsfmt("cp: -r not specified; omitting directory '%s'\n\r", src);
		return 1;
	}
	...
	if (copy_recursive(src, final_dest) < 0) {
		outsfmt("cp: failed to copy '%s' to '%s'\n\r", src, final_dest);
		return 1;
	}
