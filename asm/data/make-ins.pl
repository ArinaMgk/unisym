#!/usr/bin/perl
# Parse make-ins.dat and produce generated source code files

# Opcode prefixes which need their own opcode tables
# LONGER PREFIXES FIRST!
@disasm_prefixes = qw(0F24 0F25 0F38 0F3A 0F7A 0FA6 0FA7 0F);

# This should match MAX_OPERANDS from aasm.h
$MAX_OPERANDS = 5;
...
print STDERR "Reading make-ins.dat...\n";
...
$fname = "make-ins.dat" unless $fname = $args[0];
...
if ( !defined($output) || $output eq 'i' ) {
    ...
    print I "// Copyright: (Borrow-Data-from) NASM(1996-2009), under the 2-clause BSD license\n";
    print I "// Descripts: This file is supplementary for C/C++, while Magice builds these in.\n\n";

    print I "#ifndef _INC_AASM_LIST_INSTRUCTION\n";
    print I "#define _INC_AASM_LIST_INSTRUCTION\n\n";

    print I "#define _ASM_OPERANDS_MAXNUM 5\n";
    print I "\n";
    print I "enum _asm_opcode {\n";
    $maxlen = 0;
    foreach $i (@opcodes, @opcodes_cc) {
	print I "\tI_${i},\n";
	$len = length($i);
    ...
    print I "\tI_none = -1\n";
    print I "\n};\n\n";
    print I "#define _AASM_INST_MAX_INSLEN ", $maxlen, "\n";
    print I "#define _AASM_INST_FIRST_COND_OPCODE I_", $opcodes_cc[0], "\n\n";
    print I "#endif\n";
...
if ( !defined($output) || $output eq 'n' ) {
    ...
    print N "// Copyright: (Borrow-Data-from) NASM(1996-2009), under the 2-clause BSD license\n";



    print N "const char * const _asm_inst_literal_list[] = {";
    $first = 1;
    foreach $i (@opcodes, @opcodes_cc) {
	print N "," if ( !$first );
	$first = 0;
...
sub byte_code_compile($$) {
	...
	if ($op eq 'o16') {
	    ...
	    # This is because NASM 2.7 want this byte code at the start of
