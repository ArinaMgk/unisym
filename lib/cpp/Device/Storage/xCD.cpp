...
namespace uni {
	...
	bool xCD_ATAPI::Read(stduint BlockIden, void* Dest, stduint Times) {
		for0(t, Times) {
			...
			packet[1] = 0;
			packet[2] = (blk >> 24) & 0xFF; // LBA in Big-Endian format
			packet[3] = (blk >> 16) & 0xFF;
			packet[4] = (blk >> 8) & 0xFF;
			packet[5] = blk & 0xFF;
			...
			IN_wn(io_base + REG_DATA, (word*)dst, 2048);
