...
// gcc ./ChrMul.c ../source/hstring.c ../source/ustring.c -D_dbg -o ../../../_bin/tcm.exe
...
int main()
{
	...
	memf(p);
	// [s] +0 and -0
	aflag.Signed = 1; p = ChrMul("+0", "-0");
	printf("+0*-0=%s : sign(%c) fail(%d)\n", p, aflag.Sign ? '-' : '+', aflag.Failure);
	memf(p);
	// [s] neg and +0
	aflag.Signed = 1; p = ChrMul("-1", "0");
	printf("-1*0=%s : sign(%c) fail(%d)\n", p, aflag.Sign ? '-' : '+', aflag.Failure);
	memf(p);
	// [s] dif sign, equal
	aflag.Signed = 1; p = ChrMul("-1", "+1");
	printf("-1*+1=%s : sign(%c) fail(%d)\n", p, aflag.Sign ? '-' : '+', aflag.Failure);
	memf(p);
	// big number
	aflag.Signed = 1; p = ChrMul("-9876543210", "1234567890");
	printf("-9876543210*1234567890=%s : sign(%c) fail(%d)\n", p, aflag.Sign ? '-' : '+', aflag.Failure);
	memf(p);
...
/* Output passed
	...
	-1*0=-0 : sign(-) fail(0)
	-1*+1=-1 : sign(-) fail(0)
	-9876543210*1234567890=-12193263111263526900 : sign(-) fail(0)
