// ASCII C TAB4 CRLF
// Docutitle: (Format.Executable) [outdated] as86 (bin86 version 0.3)
// Codifiers: @ArinaMgk
...
static void as86_init(FILE * fp, ldfunc ldef, evalfunc eval)
{
	as86fp = fp;
...
static int32_t as86_section_names(rostr name, int pass, int *bits)
{
...
static int as86_add_string(char *name)
{
	...
	StrbuffSend(strs, name, (int32_t)(length + 1));
	strslen += 1 + length;
						  ...
						  int is_global, char *special)
{
	...
	if (special)
		aasm_log(_LOG_ERROR, "as86 format does not support any"
						   ...
						   int32_t segment, int32_t bytes, int relative)
{
	...
	p = sect->last = *sect->tail = malcof(struct Piece);
	sect->tail = &p->next;
	p->next = NULL;
	...
	if (type == 1 && segment == stext.index)
		...
		p->number = StrpageGet(bsym, segment), p->type = 2;
}
					 ...
					 int32_t segment, int32_t wrt)
{
	...
	if (wrt != NO_SEG) {
		...
		aasm_log(_LOG_ERROR, "WRT not supported by as86 output format");
	}
	...
	if (type == OUT_RESERVE) {
		...
		as86_sect_write(s, (uint8*)data, size);
		as86_add_piece(s, 0, 0L, 0L, size, 0);
	} else if (type == OUT_ADDRESS) {
		if (segment != NO_SEG) {
			if (segment % 2) {
				aasm_log(_LOG_ERROR, "as86 format does not support"
...
static void as86_set_rsize(int size)
{
	if (as86_reloc_size != size) {
		...
		case 4:
			...
			break;
		default:
			aasm_log(_LOG_PANIC, "bizarre relocation size %d", size);
		}
	}
}
	...
	for (p = sect->head; p; p = p->next)
		...
		case 0:
			...
			do {
				...
				StrbuffPop(sect->data, buf, tmplen);
...
static void as86_filename(char *inname, char *outname)
{
	...
	if ((p = (char*)StrIndexCharRight(inname, '.')) != NULL) {
		StrCopyN(as86_module, inname, p - inname);
		as86_module[p - inname] = '\0';
	} else
		StrCopy(as86_module, inname);

	standard_extension(inname, outname, ".o");
}

_ESYM_C macros_t as86_stdmac[];
