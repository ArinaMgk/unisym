// ASCII C TAB4 CRLF
// Docutitle: (Format.Executable) Common Object File Format (for DJGPP and Win32)
// Codifiers: @ArinaMgk
...
static Strpage *bsym, *symval;

static Strbuff *strs;
static uint32_t strslen;

static void coff_gen_init(FILE *);
...
static void coff_win32_init(FILE * fp,
...
static void coff_win64_init(FILE * fp,
							ldfunc ldef, evalfunc eval)
{
	extern struct outffmt of_win64;
	...
	(void)eval;
	coff_gen_init(fp);
	imagebase_sect = SegAlloc()+1;
	ldef(WRT_IMAGEBASE,imagebase_sect,0,NULL,false,false,&of_win64);
}

static void coff_std_init(FILE * fp, ldfunc ldef,
						  evalfunc eval)
{
	win32 = win64 = false;
	(void)ldef;                 /* placate optimizers */
...
static void coff_gen_init(FILE * fp)
{

	coffp = fp;
	sects = NULL;
	nsects = sectlen = 0;
	syms = StrbuffNew(sizeof(struct Symbol));
	nsyms = 0;
	bsym = StrpageNew();
	symval = StrpageNew();
	strs = StrbuffNew(1);
...
static int coff_make_section(char *name, uint32_t flags)
{
	...
	if (!StrCompare(name, ".text"))
		...
		s->index = SegAlloc();
	StrCopyN(s->name, name, 8);
	s->name[8] = '\0';
	s->flags = flags;

	if (nsects >= sectlen) {
	sectlen += SECT_DELTA;
		sects = (Section**)aasm_realloc(sects, sectlen * sizeof(*sects));
	}
	sects[nsects++] = s;
...
static int32_t coff_section_names(rostr name, int pass, int *bits)
{
	char *p;
	...
	if (*p)
		*p++ = '\0';
	if (StrLength(name) > 8) {
		aasm_log(_LOG_WARN, "COFF section names limited to 8 characters: truncating");
		uni::cast<char*>(name)[8] = '\0';
	}
	flags = 0;
	...
	while (*p) {
		...
		if (!StrCompareInsensitive(q, "code") || !StrCompareInsensitive(q, "text")) {
			flags = TEXT_FLAGS;
		} else if (!StrCompareInsensitive(q, "data")) {
			flags = DATA_FLAGS;
		} else if (!StrCompareInsensitive(q, "rdata")) {
			if (win32 | win64)
				flags = RDATA_FLAGS;
			else {
				flags = DATA_FLAGS;     /* gotta do something */
				aasm_log(_LOG_ERROR, "standard COFF does not support"
					  " read-only data sections");
			}
		} else if (!StrCompareInsensitive(q, "bss")) {
			flags = BSS_FLAGS;
		} else if (!StrCompareInsensitive(q, "info")) {
			if (win32 | win64)
				flags = INFO_FLAGS;
			else {
				flags = DATA_FLAGS;     /* gotta do something */
		...
		} else if (!StrCompareNInsensitive(q, "align=", 6)) {
			if (!(win32 | win64))
				aasm_log(_LOG_ERROR, "standard COFF does not support"
					  " section alignment specification");
			else {
				if (q[6 + StrSpanInclude(q + 6, "0123456789")])
					aasm_log(_LOG_ERROR,
						  "argument to `align' is not numeric");
				else {
					unsigned int align = atoins(q + 6);
					if (!align || ((align - 1) & align))
						aasm_log(_LOG_ERROR, "argument to `align' is not a"
							  " power of two");
					else if (align > 64)
						aasm_log(_LOG_ERROR, "Win32 cannot align sections"
	...
	if (i == nsects) {
		if (!flags) {
			if (!StrCompare(name, ".data"))
				flags = DATA_FLAGS;
			else if (!StrCompare(name, ".rdata"))
				flags = RDATA_FLAGS;
			else if (!StrCompare(name, ".bss"))
				flags = BSS_FLAGS;
		else if (win64 && !StrCompare(name, ".pdata"))
		flags = 0x40300040; /* rdata align=4 */
		else if (win64 && !StrCompare(name, ".xdata"))
		flags = 0x40400040; /* rdate align=8 */
			else
				flags = TEXT_FLAGS;
		}
		i = coff_make_section((char*)name, flags);
		if (flags)
			sects[i]->flags = flags;
						  ...
						  int is_global, char *special)
{
	...
	if (special)
		aasm_log(_LOG_ERROR, "COFF format does not support any"
			  " special symbol types");

	if (name[0] == '.' && name[1] == '.' && name[2] != '@') {
	if (StrCompare(name,WRT_IMAGEBASE))
			aasm_log(_LOG_ERROR, "unrecognized special symbol `%s'", name);
		return;
	}

	if (StrLength(name) > 8) {
	size_t nlen = StrLength(name)+1;
		StrbuffSend(strs, name, nlen);
	strslen += nlen;
	} else
		pos = -1;

	sym = (Symbol*)StrbuffEalloc(syms);

	sym->strpos = pos;
	sym->namlen = StrLength(name);
	if (pos == -1)
	 ...
	 */
	if (sym->section == 0) {
		bsym = StrpageSet(bsym, segment, nsyms);
	}

	if (segment != NO_SEG)
		symval = StrpageSet(symval, segment, sym->section ? 0 : sym->value);
				...
				int16_t type)
{
	...
	r->address = sect->len;
	if (segment == NO_SEG)
		r->symbol = 0, r->symbase = Reloc::ABS_SYMBOL;
	else {
		int i;
		r->symbase = Reloc::REAL_SYMBOLS;
		for (i = 0; i < nsects; i++)
			if (segment == sects[i]->index) {
				r->symbol = i * 2;
				r->symbase = Reloc::SECT_SYMBOLS;
				break;
			}
		if (r->symbase == Reloc::REAL_SYMBOLS)
			r->symbol = StrpageGet(bsym, segment);
	}
	r->type = type;
	 ...
	 */
	if (r->symbase == Reloc::REAL_SYMBOLS && !(win32 | win64))
		return StrpageGet(symval, segment);
	else
		return 0;
}
					 ...
					 int32_t segment, int32_t wrt)
{
	...
	if (wrt != NO_SEG && !win64) {
		...
		aasm_log(_LOG_ERROR, "WRT not supported by COFF output formats");
	}
	...
	if (!s) {
		int tempint;            /* ignored */
		if (segto != coff_section_names((char*)".text", 2, &tempint))
			aasm_log(_LOG_PANIC, "strange segment conditions in COFF driver");
		else
			s = sects[nsects - 1];
	}
	...
	if (win64 && wrt == NO_SEG &&
	(!StrCompare(s->name,".pdata") || !StrCompare(s->name,".xdata")))
	wrt = imagebase_sect;
	...
	if (type == OUT_RESERVE) {
		if (s->data) {
			aasm_log(_LOG_WARN, "uninitialised space declared in"
		...
		coff_sect_write(s, (uint8*)data, size);
	} else if (type == OUT_ADDRESS) {
		if (!(win64)) {
			if (size != 4 && (segment != NO_SEG || wrt != NO_SEG))
				aasm_log(_LOG_ERROR, "COFF format does not support non-32-bit"
					  " relocations");
			else {
				int32_t fix = 0;
				if (segment != NO_SEG || wrt != NO_SEG) {
					if (wrt != NO_SEG) {
						aasm_log(_LOG_ERROR, "COFF format does not support"
							  " WRT types");
					} else if (segment % 2) {
						aasm_log(_LOG_ERROR, "COFF format does not support"
			...
			aasm_log(_LOG_ERROR, "operand size mismatch: 'wrt "
		...
		aasm_log(_LOG_ERROR, "COFF format does not support 16-bit"
			  " relocations");
		...
		if (segment == segto && !(win64))  /* Acceptable for RIP-relative */
			...
			aasm_log(_LOG_ERROR, "Win32 COFF does not correctly support"
...
static void coff_sect_write(Section *sect, const uint8_t *data, uint32_t len)
{
...
void AddExport(char *name)
{
	...
	newS = (STRING *) malc(sizeof(STRING));
	newS->len = StrLength(name);
	newS->Next = NULL;
	newS->String = (char *)malc(newS->len + 1);
	StrCopy(newS->String, name);
	if (rvp == NULL) {
		int i;
		for (i = 0; i < nsects; i++)

			if (!StrCompare(EXPORT_SECTION_NAME, sects[i]->name))
				break;
		if (i == nsects)
			directive_sec =
				sects[coff_make_section
					  ((char*)EXPORT_SECTION_NAME, EXPORT_SECTION_FLAGS)];
		else
			directive_sec = sects[i];
		Exports = newS;
	} else {
		while (rvp->Next) {
			if (!StrCompare(rvp->String, name))
				return;
			rvp = rvp->Next;
		}
		rvp->Next = newS;
...
static int coff_directives(char *directive, char *value, int pass)
{
	if (!StrCompare(directive, "export")) {
		...
		if (*q) {
			aasm_log(_LOG_ERROR, "unrecognized export qualifier `%s'", q);
			return 1;
		}
		AddExport(name);
		return 1;
	} else if (win32 && !StrCompare(directive,"safeseh")) {
	static int sxseg=-1;
	int i;
	if (sxseg==-1)
	{   for (i = 0; i < nsects; i++)
		if (!StrCompare(".sxdata",sects[i]->name))
			break;
		if (i == nsects)
		sxseg = coff_make_section((char*)".sxdata",0x200);
		else
		sxseg = i;
	}
	...
	if (pass0 == 2) {
		...
		StrbuffRewind(syms);
		for (n = 0; n < nsyms; n++) {
		Symbol *sym = (Symbol*)StrbuffEread(syms);
		bool equals;

		/* sym->strpos is biased by 4, because symbol
		 * table is prefixed with table length */
		if (sym->strpos >=4) {
			char *name = (char*)malc(sym->namlen+1);
			StrbuffPopx(strs, sym->strpos-4, name, sym->namlen);
			name[sym->namlen] = '\0';
			equals = !StrCompare(value,name);
			memf(name);
		}
		else
			equals = !StrCompare(value,sym->name);
...
static void coff_write(void)
{
	...
	StrbuffRewind(syms);
	for (n = 0; n < nsyms; n++) {
			struct Symbol *sym = (Symbol*)StrbuffEread(syms);
		if (sym->strpos == -1 && !StrCompare("@feat.00",sym->name))
		break;
	}
	if (n == nsyms)
		coff_deflabel((char*)"@feat.00", NO_SEG, 1, 0, NULL);
	}
	...
	for (i = 0; i < nsects; i++)
		if (sects[i]->data) {
			StrbuffSendFile(sects[i]->data, coffp);
...
static void coff_write_relocs(struct Section *s)
{
	...
	for (r = s->head; r; r = r->next) {
		...
		fwriteint32_t(r->symbol + (r->symbase == Reloc::REAL_SYMBOLS ? initsym :
								r->symbase == Reloc::ABS_SYMBOL ? initsym - 1 :
								r->symbase == Reloc::SECT_SYMBOLS ? 2 : 0),
				   coffp);
	fwriteint16_t(r->type, coffp);
	}
}

static void coff_symbol(rostr name, int32_t strpos, int32_t value, int section, int type, int storageclass, int aux)
{
	char padname[8];
...
static void coff_write_symbols(void)
{
	...
	StrCopyN(filename, coff_infile, 18);
