// ASCII C TAB4 CRLF
// Docutitle: (Format.Executable) IEEE Object Format
// Codifiers: @ArinaMgk
...
/* notes: NASM have tried to make this correspond to the IEEE version
...
static int32_t ieee_segment(rostr, int, int *);
...
static void ieee_putascii(rostr, ...);
...
void dbgls_init(outffmt *of, void *id, FILE *fp);
static void dbgls_linnum(const char *lnfname, int32_t lineno, int32_t segto);
static void dbgls_deflabel(char *name, int32_t segment, int64_t offset, int is_global, char *special);
static void dbgls_typevalue(int32_t type);
static void dbgls_output(int output_type, void *param);
static void dbgls_cleanup();

static void ieee_init(FILE *fp, ldfunc ldef, evalfunc eval);
static int ieee_set_info(geninfo type, char **val);
static void ieee_out(int32_t segto, const void *data, out_type type, uint64_t size, int32_t segment, int32_t wrt);
static void ieee_deflabel(char *name, int32_t segment, int64_t offset, int is_global, char *special);
...
static struct dbgffmt ladsoft_debug_form = {
...
static struct dbgffmt* ladsoft_debug_arr[3] = {
	&ladsoft_debug_form,
	&null_debug_form,
	NULL
};
struct outffmt of_ieee = {
						  ...
						  int64_t offset, int is_global, char *special)
{
	...
	if (special) {
		aasm_log(_LOG_ERROR, "unrecognised symbol type `%s'", special);
	}
	/*
	...
	if (segment >= SEG_ABS) {
		...
		if (is_global) {
			...
			pub = *fpubtail = (ieeePublic*)malc(sizeof(*pub));
			fpubtail = &pub->next;
			pub->next = NULL;
			pub->name = name;
			pub->offset = offset;
	...
	for (seg = seghead; seg && is_global; seg = seg->next)
		if (seg->index == segment) {
			...
			last_defined = pub = *seg->pubtail = (ieeePublic*)malc(sizeof(*pub));
	 ...
	 */
	if (is_global) {
		ext = *exttail = (ieeeExternal*)malc(sizeof(*ext));
		ext->next = NULL;
		exttail = &ext->next;
		ext->name = name;
		if (is_global == 2)
					 ...
					 int32_t segment, int32_t wrt)
{
	...
	if (!any_segs) {
		...
		if (segto != ieee_segment("__NASMDEFSEG", 2, &tempint))
			aasm_log(_LOG_PANIC, "strange segment conditions in IEEE driver");
	}
...
static void ieee_data_new(struct ieeeSection *segto)
{
	...
	if (!segto->data)
		segto->data = segto->datacurr = (ieeeObjData*)
			malc(sizeof(*(segto->datacurr)));
	else
		segto->datacurr = segto->datacurr->next = (ieeeObjData*)
			malc(sizeof(*(segto->datacurr)));
							 ...
							 uint64_t realtype, int32_t offset)
{
	...
	s.ftype = (ieeeFixupp::ftype_t) - 1;
	/* if it is a WRT offset */
	if (wrt != NO_SEG) {
		s.ftype = ieeeFixupp::FT_WRT;
		s.addend = offset;
		if (wrt >= SEG_ABS)
			s.id1 = -(wrt - SEG_ABS);
		else {
			if (wrt % 2 && realtype != OUT_REL2ADR
				...
				aasm_log(_LOG_ERROR, "target of WRT must be a section ");
		}
		s.size = size;
	...
	if (segment != NO_SEG) {
		...
		if (segment >= SEG_ABS) {
			...
			if (target)
				...
				if (eb) {
					...
					aasm_log(_LOG_PANIC, "unrecognised segment value in ieee_write_fixup");
			}
			...
			if (target) {
				if (realtype == OUT_REL2ADR || realtype == OUT_REL4ADR) {
					...
					s.id1 = target->ieee_index;
					s.ftype = ieeeFixupp::FT_REL;
					s.size = size;
					s.addend = offset;
				} else {
					/* We were offsetting from a seg */
					s.id1 = target->ieee_index;
					s.ftype = ieeeFixupp::FT_OFS;
					s.size = size;
					s.addend = offset;
				}
			} else {
				...
				if (eb) {
					if (realtype == OUT_REL2ADR || realtype == OUT_REL4ADR) {
						s.ftype = ieeeFixupp::FT_EXTREL;
						s.addend = 0;
						s.id1 = eb->index[i];
					} else {
						/* else we want the external offset */
						s.ftype = ieeeFixupp::FT_EXT;
						s.addend = 0;
						s.id1 = eb->index[i];
					}
		...
		if (size != 2 && s.ftype == ieeeFixupp::FT_SEG)
			aasm_log(_LOG_ERROR, "IEEE format can only handle 2-byte"
...
static void ieee_install_fixup(ieeeSection *seg, ieeeFixupp *fix)
{
	ieeeFixupp *f;
	f = malcof(struct ieeeFixupp);
	MemCopyN(f, fix, sizeof(struct ieeeFixupp));
 ...
 * segment registry
 */
static int32_t ieee_segment(rostr name, int pass, int *bits)
{
	/*
	...
	bool rn_error = 0;
		...
		*segtail = seg = (ieeeSection*)malc(sizeof(*seg));
		seg->next = NULL;
		segtail = &seg->next;
		seg->index = SegAlloc();
		...
		seg->combine = ieeeSection::CMB_PUBLIC;      /* default */
		seg->pubhead = NULL;
		...
		while (attrs--) {
			...
			if (!StrCompareInsensitive(p, "private"))
				seg->combine = ieeeSection::CMB_PRIVATE;
			else if (!StrCompareInsensitive(p, "public"))
				seg->combine = ieeeSection::CMB_PUBLIC;
			else if (!StrCompareInsensitive(p, "common"))
				seg->combine = ieeeSection::CMB_COMMON;
...
static void ieee_write_file(int debuginfo)
{
	 ...
	 */
	ieee_putascii("MBFNASM,%02X%s.\n", StrLength(ieee_infile), ieee_infile);

	/*
	 * Write the NASM boast comment.
	 */
	ieee_putascii("CO0,%02X%s.\n", StrLength(aasm_comment), aasm_comment);
	 ...
	 */
	seg = seghead;
	if (!debuginfo && !StrCompare(seg->name, "??LINE"))
		seg = seg->next;
	while (seg) {
		char buf[256];
		char attrib;
		switch (seg->combine) {
		case ieeeSection::CMB_PUBLIC:
		default:
			attrib = 'C';
			break;
		case ieeeSection::CMB_PRIVATE:
			attrib = 'S';
			break;
		case ieeeSection::CMB_COMMON:
			attrib = 'M';
			break;
		}
	...
	if (ieee_entry_seg) {
		for (seg = seghead; seg; seg = seg->next)
			if (seg->index == ieee_entry_seg)
				break;
		if (!seg)
			aasm_log(_LOG_PANIC, "Start address records are incorrect");
		else
...
static void ieee_putascii(rostr format, ...)
{
	char buffer[256];
	int i, l;
	va_list ap;
	...
	case ieeeFixupp::FT_WRT:
		if (p->id2 < 0)
			...
			sprintf(buf, "R%" PRIX32 ",%" PRIX32 ",+,L%" PRIX32 ",+,L%" PRIX32 ",-", p->id2, p->addend,
					p->id2, p->id1);
		break;
	case ieeeFixupp::FT_EXT:
		sprintf(buf, "X%" PRIX32 "", p->id1);
		break;
	case ieeeFixupp::FT_EXTREL:
		sprintf(buf, "X%" PRIX32 ",P,-,%" PRIX32 ",-", p->id1, size);
		break;
	case ieeeFixupp::FT_EXTSEG:
...
static void ieee_unqualified_name(char *dest, char *source)
{
	if (ieee_uppercase) {
		while (*source)
			*dest++ = ascii_toupper(*source++);
		*dest = 0;
	} else
		StrCopy(dest, source);
}
void dbgls_init(struct outffmt *of, void *id, FILE * fp)
{
	int tempint;
	(void)of;
	(void)id;
	...
	arrhead = NULL;
	arrtail = &arrhead;
	ieee_segment((char*)"??LINE", 2, &tempint);
	any_segs = false;
}
...
static void dbgls_linnum(const char *lnfname, int32_t lineno, int32_t segto)
{
	...
	if (!any_segs) {
		int tempint;            /* ignored */
		if (segto != ieee_segment((char*)"__NASMDEFSEG", 2, &tempint))
						   ...
						   int64_t offset, int is_global, char *special)
{
	...
	for (seg = seghead; seg; seg = seg->next)
		if (seg->index == segment) {
			 ...
			 */
			if (!is_global) {
				last_defined = loc = (ieeePublic*)malc(sizeof(*loc));
				*seg->loctail = loc;
