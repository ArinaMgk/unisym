...
void (*fp_normal)(rostr);
Handler_t fp_over_digits;
Handler_t fp_fn1;
Handler_t fp_fn_underflow, fp_fn_overflow, fp_fn_denormal;

// ---- ---- local variables ---- ---- //
static bool daz = false;// denormals as zero
static enum float_round_t rc = FLOAT_RC_NEAR; // rounding control
...
#define SOME_ARG(a,i) (a)[(i)+0], (a)[(i)+1], (a)[(i)+2], (a)[(i)+3], (a)[(i)+4], (a)[(i)+5]
	...
	while (*string) {
		if (*string >= '0' && *string <= '9') {
		 ...
		 /* do nothing */
			...
			asserv(fp_normal)(String::newFormat("invalid character in floating-point constant %s: '%c'", "exponent", *string).reference());
			return INT32_MAX;
		}
		string++;
	}
 ...
 * ---------------------------------------------------------------------------
 */
static bool ieee_flconvert(const char* string, fp_limb* mant, int32_t* exponent)
{
	...
	while (*string && *string != 'E' && *string != 'e') {
		if (*string == '.') {
			if (!seendot) {
				...
				asserv(fp_normal)("too many periods in floating-point constant");
				return false;
			}
		}
		 ...
		 /* do nothing */
			...
			asserv(fp_normal)(String::newFormat("invalid character in floating-point constant %s: '%c'", "significand", *string).reference());
			return false;
		}
		string++;
	}
...
static bool ieee_flconvert_bin(const char *string, int bits, fp_limb *mant, int32_t *exponent)
{
	...
	while ((c = *string++) != '\0') {
		...
		/* ignore */
			...
			asserv(fp_normal)(String::newFormat("floating-point constant: `%c' is invalid character", c).reference());
			return false;
		}
	}
	...
	if (!seendigit) {
		...
		*exponent = 0;
		...
		MemCopyN(mant, mult, MANT_LIMBS*sizeof(fp_limb));
		*exponent = twopwr;
	}
...
/* Produce standard IEEE formats, with implicit or explicit_int integer
...
struct ieee_format {
	...
	int explicit_int;// explicit_int integer, "explicit" is a cpp keyword
...
static int to_packed_bcd(const char* str, const char* p, int s, uint8_t* result, const struct ieee_format* fmt)
{
	int n = 0;
	char c;
	int tv = -1;
	...
	if (fmt != &ieee_80) {
		asserv(fp_normal)("packed BCD requires an 80-bit format");
		return 0;
	}
	...
	while (p >= str) {
		...
		if (c >= '0' && c <= '9') {
			if (tv < 0) {
				if (n == 9) {
					asserv(fp_normal)("packed BCD truncated to 18 digits");
				}
				tv = c - '0';
			}
			else {
		 ...
		 /* do nothing */
			...
			asserv(fp_normal)(String::newFormat("invalid character `%c' in packed BCD constant", c).reference());
			return 0;
		}
	}
	if (tv >= 0) {
...
static int to_float(const char* str, int s, uint8_t* result, const struct ieee_format* fmt)
{
	fp_limb mant[MANT_LIMBS];
	int32_t exponent = 0;
	const int32_t expmax = 1 << (fmt->exponent - 1);
	fp_limb one_mask = LIMB_TOP_BIT >>
		((fmt->exponent + fmt->explicit_int) % LIMB_BITS);
	const int one_pos = (fmt->exponent + fmt->explicit_int) / LIMB_BITS;
	int i;
	int shift;
	enum floats type;
	bool ok;
	...
	if (!str[0]) {
		asserv(fp_normal)("internal errror: empty string passed to float_const");
		return 0;
	}

	strend = StrIndexChar(str, '\0');
	...
	/* Special tokens */
		...
		case 'I':
			...
			break;
		default:
			asserv(fp_normal)(String::newFormat("internal error: unknown FP constant token `%s'\n", str).reference());
			type = FL_QNAN;
			break;
		}
	}
	...
	case FL_DENORMAL:
	{
		...
		if (mant[one_pos] & one_mask) {
			...
			exponent = 1;
			if (!fmt->explicit_int)
				mant[one_pos] &= ~one_mask;	/* remove explicit_int one */
			mant[0] |= exponent << (LIMB_BITS - 1 - fmt->exponent);
		}
		else {
			if (daz || is_zero(mant)) {
				/* Flush denormals to zero */
				asserv(fp_fn_underflow)();
				goto zero;
			}
			else {
				asserv(fp_fn_denormal)();
			}
		}
		break;
	}
	...
	case FL_NORMAL:
		exponent += expmax - 1;
		ieee_shr(mant, fmt->exponent + fmt->explicit_int);
		ieee_round(minus, mant, bits);
		/* did we scale up by one? */
		if (test_bit(mant, fmt->exponent + fmt->explicit_int - 1)) {
			ieee_shr(mant, 1);
			exponent++;
			if (exponent >= (expmax << 1) - 1) {
				asserv(fp_fn_overflow)();
				type = FL_INFINITY;
				goto overflow;
			}
		}
	...
	overflow:
		...
		if (type == FL_QNAN)
			set_bit(mant, fmt->exponent + fmt->explicit_int + 1);
		else if (type == FL_SNAN)
			set_bit(mant, fmt->exponent + fmt->explicit_int + fmt->mantissa);
		break;
	}
...
{
	String str;//{TODO} add a macro-option
	switch (bytes) {
	case 1:
	...
	default:
		// no pass1
		str.Format("float_const: invalid number of bytes %d", bytes);
		asserv(fp_normal)(str.reference());
		return 0;
	}
}
...
{
	if (!StrCompareInsensitive(option, "daz")) {
		daz = true;
	}
	else if (!StrCompareInsensitive(option, "nodaz")) {
		daz = false;
	}
	else if (!StrCompareInsensitive(option, "near")) {
		rc = FLOAT_RC_NEAR;
	}
	else if (!StrCompareInsensitive(option, "down")) {
		rc = FLOAT_RC_DOWN;
	}
	else if (!StrCompareInsensitive(option, "up")) {
		rc = FLOAT_RC_UP;
	}
	else if (!StrCompareInsensitive(option, "zero")) {
		rc = FLOAT_RC_ZERO;
	}
	else if (!StrCompareInsensitive(option, "default")) {
		rc = FLOAT_RC_NEAR;
		daz = false;
	}
	else {
