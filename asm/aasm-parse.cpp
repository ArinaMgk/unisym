...
_ESYM_C int in_abs_seg;          /* ABSOLUTE segment flag */
_ESYM_C int32_t abs_seg;            /* ABSOLUTE segment */
_ESYM_C int32_t abs_offset;         /* ABSOLUTE segment offset */

static int is_comma_next(void);

static int i;
static struct tokenval tokval;
static struct outffmt *outfmt;     /* Structure of addresses of output routines */
static struct location *location;         /* Pointer to current line's segment,offset */

void Parser::parser_global_info(outffmt *output, struct location * locp)
{
	outfmt = output;
	location = locp;
}
...
static int prefix_slot(enum prefixes prefix)
{
	...
	default:
		//--auto clean flag
		aasm_log(_LOG_PANIC, "Invalid value %d passed to prefix_slot()", prefix);
		return -1;
	}
}
...
static void process_size_override(insn* result, int operand)
{
	if (tasm_compatible_mode) {
		...
		default:
			//--auto clean flag
			aasm_log(_LOG_ERROR, "invalid operand size specification");
			break;
		}
	}
	else {
		...
		case P_A64:
			...
			{
				//--auto clean flag
				aasm_log(_LOG_ERROR, "conflicting address size specifications");
			}
			else
				result->prefixes[PPS_ASIZE] = (prefixes)tokval.t_integer;
			break;
		case S_WORD:
		...
		default:
			//--auto clean flag
			aasm_log(_LOG_ERROR, "invalid size specification in effective address");
			break;
		}
	}
}

insn* Parser::parse_line(int pass, char* buffer, insn* result, evalfunc evaluate, ldfunc ldef)
{
	int operand;
	int critical;
...
restart_parse:
	...
	if (i == TOKEN_ID || (insn_is_label && i == TOKEN_INSN)) {
		...
		if (i == ':') {         /* skip over the optional colon */
			...
			//--auto clean flag
			log_pass1 = 1;
			log_warnt = ERR_WARN_OL;
			aasm_log(_LOG_WARN, "label alone on a line without a colon might be in error");
		}
		if (i != TOKEN_INSN || tokval.t_integer != I_EQU) {
			...
			ldef(result->label, in_abs_seg ? abs_seg : location->segment,
				location->offset, NULL, true, false, outfmt);
		}
	}
		...
		(i == TOKEN_REG && !(REG_SREG & ~aasm_reg_flags[tokval.t_integer])))
	{
		...
		if (i == TOKEN_PREFIX && tokval.t_integer == P_TIMES) {
			...
			if (!is_simple(value)) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "non-constant argument supplied to TIMES");
				result->times = 1L;
			}
			else {
				result->times = value->value;
				if (value->value < 0 && pass0 == 2) {
					aasm_log(_LOG_ERROR, "TIMES value %d is negative", value->value);
					result->times = 0;
				}
			}
		}
		else {
			int slot = prefix_slot((prefixes)tokval.t_integer);
			if (result->prefixes[slot]) {
				if (result->prefixes[slot] == tokval.t_integer)
				{
					//--auto clean flag
					aasm_log(_LOG_WARN, "instruction has redundant prefixes");
				}
				else
				{
					//--auto clean flag
					aasm_log(_LOG_ERROR, "instruction has conflicting prefixes");
				}
			}
			result->prefixes[slot] = (prefixes)tokval.t_integer;
			i = stdscan(NULL, &tokval);
		}
	}
	...
	if (i != TOKEN_INSN) {
		...
		if (i == 0 && pfx != P_none) {
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "parser: instruction expected");
			result->opcode = I_none;
			return result;
		}
	}

	result->opcode = (_asm_opcode)tokval.t_integer;
	result->condition = (ccode)tokval.t_inttwo;
	...
	if (result->opcode == I_DB || result->opcode == I_DW ||
		...
		while (1) {
			...
			first = false;
			fixptr = tail;
			eop = *tail = (extop*)malc(sizeof(extop));
			tail = &eop->next;
			eop->next = NULL;
			...
			if (i == TOKEN_STR && is_comma_next()) {
				...
				enum strfunc func = (enum strfunc)tokval.t_integer;
				i = stdscan(NULL, &tokval);
				if (i == '(') {
					parens = true;
					i = stdscan(NULL, &tokval);
				...
				if (i != TOKEN_STR) {
					//--auto clean flag
					aasm_log(_LOG_ERROR, "%s must be followed by a string constant", funcname);
					eop->type = EOT_NOTHING;
				}
				else {
					...
					if (eop->stringlen == (size_t)-1) {
						//--auto clean flag
						aasm_log(_LOG_ERROR, "invalid string for transform");
						eop->type = EOT_NOTHING;
					}
				}
				if (parens && i && i != ')') {
					...
					if (i != ')') {
						//--auto clean flag
						aasm_log(_LOG_ERROR, "unterminated %s function", funcname);
					}
				}
				if (i && i != ',')
					i = stdscan(NULL, &tokval);
				...
				if (i != TOKEN_FLOAT) {
					stdscan_bufptr = save;
					i = tokval.t_type = (token_type)token;
					goto is_expression;
				}
				else {
					goto is_float;
			...
			is_float:
				...
				case I_DY:
					//--auto clean flag
					aasm_log(_LOG_ERROR, "floating-point constant encountered in DY instruction");
					eop->stringlen = 0;
					break;
				default:
					//--auto clean flag
					aasm_log(_LOG_ERROR, "floating-point constant encountered in unknown instruction");
					/*
				...
				eop = (extop*)aasm_realloc(eop, sizeof(extop) + eop->stringlen);
				tail = &eop->next;
				*fixptr = eop;
				eop->stringval = (char*)eop + sizeof(extop);
				if (!eop->stringlen ||
					!float_const(tokval.t_charptr, sign, (uint8_t*)eop->stringval, eop->stringlen))
					eop->type = EOT_NOTHING;
				i = stdscan(NULL, &tokval); /* eat the comma */
			}
			else {
		 /* anything else, assume it is an expression */
			...
			is_expression:
				value = evaluate(stdscan, NULL, &tokval, NULL, critical, NULL);
				i = tokval.t_type;
				...
				if (is_unknown(value)) {
					...
					//--auto clean flag
					aasm_log(_LOG_ERROR, "operand %d: expression is not simple or relocatable", oper_num);
				}
			}
			...
			if (i != ',') {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "comma expected after operand %d", oper_num);
				result->opcode = I_none;    /* unrecoverable parse error: */
				return result;  /* ignore this instruction */
			}
		}
		...
		if (result->opcode == I_INCBIN) {
			...
			if (!result->eops || result->eops->type != EOT_DB_STRING)
				aasm_log(_LOG_ERROR, "`incbin' expects a file name");
			else if (result->eops->next &&
				result->eops->next->type != EOT_DB_NUMBER)
				aasm_log(_LOG_ERROR, "`incbin': second parameter is", " non-numeric");
			else if (result->eops->next && result->eops->next->next &&
				result->eops->next->next->type != EOT_DB_NUMBER)
				aasm_log(_LOG_ERROR, "`incbin': third parameter is", " non-numeric");
			else if (result->eops->next && result->eops->next->next &&
				result->eops->next->next->next)
				aasm_log(_LOG_ERROR, "`incbin': more than three parameters");
			else
				return result;
			/*
		...
		else /* DB ... */ if (oper_num == 0)
		{
			//--auto clean flag
			log_pass1 = 1;
			aasm_log(_LOG_WARN, "no operand for data declaration");
		}
		else
	...
	for (operand = 0; operand < MAX_OPERANDS; operand++) {
		...
		if ((result->oprs[operand].type & FAR) && !mref &&
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "invalid use of FAR operand specifier");
		}
		...
		if (i == ':' && mref) { /* it was seg:offset */
			...
			if (value[1].type != 0 || value->value != 1 ||
				...
				//--auto clean flag
				aasm_log(_LOG_ERROR, "instruction has conflicting segment overrides");
			}
			else {
		...
		if (mref && bracket) {  /* find ] at the end */
			if (i != ']') {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "parser: expecting ]");
				recover = true;
			}
				...
				if (i != 0 && i != ',') {
					//--auto clean flag
					aasm_log(_LOG_ERROR, "comma or end of line expected");
					recover = true;
				}
			}
		}
			...
			if (i != 0 && i != ',' && i != ':') {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "comma, colon or end of line expected");
				recover = true;
			}
			else if (i == ':') {
		...
		if (mref) {             /* it's a memory reference */
			...
			result->oprs[operand].hinttype = (eval_hint)hints.type;
			...
			if (e->type && e->type <= EXPR_REG_END) {   /* it's a 2nd register */
				if (b != -1)    /* If the first was the base, ... */
					...
					//--auto clean flag
					aasm_log(_LOG_ERROR, "beroset-p-592-invalid effective address");
					result->opcode = I_none;
					return result;
				}
				else
			...
			if (e->type != 0) { /* is there an offset? */
				if (e->type <= EXPR_REG_END) {  /* in fact, is there an error? */
					//--auto clean flag
					aasm_log(_LOG_ERROR, "beroset-p-603-invalid effective address");
					result->opcode = I_none;
					return result;
				}
				else {
					if (e->type == EXPR_UNKNOWN) {
						...
						if (e->type && e->type < EXPR_SEGBASE) {
							//--auto clean flag
							aasm_log(_LOG_ERROR, "beroset-p-630-invalid effective address");
							result->opcode = I_none;
							return result;
						}
						...
						if (e->type && e->value != 1) {
							//--auto clean flag
							aasm_log(_LOG_ERROR, "beroset-p-637-invalid effective address");
							result->opcode = I_none;
							return result;
						}
						if (e->type) {
						...
						if (e->type) {
							//--auto clean flag
							aasm_log(_LOG_ERROR, "beroset-p-650-invalid effective address");
							result->opcode = I_none;
							return result;
						}
					}
			...
			if (e->type != 0) { /* there'd better be nothing left! */
				//--auto clean flag
				aasm_log(_LOG_ERROR, "beroset-p-663-invalid effective address");
				result->opcode = I_none;
				return result;
			}
		...
		/* It is memory, but it can match any r/m operand */
			...
			result->oprs[operand].basereg = (reg_enum)b;
			result->oprs[operand].indexreg = (reg_enum)i;
			...
			if (is_just_unknown(value)) {       /* it's immediate but unknown */
				...
				if (value->type >= EXPR_SIMPLE || value->value != 1) {
					//--auto clean flag
					aasm_log(_LOG_ERROR, "invalid operand type");
					result->opcode = I_none;
					return result;
				}
				...
				if (rs && (result->oprs[operand].type & SIZE_MASK) != rs) {
					//--auto clean flag
					log_pass1 = 1;
					aasm_log(_LOG_WARN, "register size specification ignored");
				}
			}
		}
	}
...
/* clear remaining operands */
	...
	switch (result->opcode) {
	case I_RESW:
		result->opcode = (_asm_opcode)I_RESB;
...
void Parser::cleanup_insn(insn* i)
{
	extop* e;
