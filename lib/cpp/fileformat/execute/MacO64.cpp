// ASCII C TAB4 CRLF
// Docutitle: (Format.Executable) NeXTstep/OpenStep/Rhapsody/Darwin/MacOS X (x86_64) object files
// Codifiers: @ArinaMgk
...
_ESYM_C void qsort(void *base, size_t nitems, size_t size, int (*compar)(const void *, const void *));


#define xStrCopyN(xdst, xsrc)						\
	MemSet(xdst, '\0', sizeof(xdst));	/* zero out whole buffer */	\
	StrCopyN(xdst, xsrc, sizeof(xdst));	/* copy over string */		\
...
_ESYM_C macros_t macho_stdmac[];

static void macho_init(FILE *fp, ldfunc ldef, evalfunc eval);
static int macho_setinfo(geninfo type, char **val);
static void macho_output(int32_t secto, const void *data, out_type type, uint64_t size, int32_t section, int32_t wrt);
static void macho_symdef(char *name, int32_t section, int64_t offset, int is_global, char *special);
static int32_t macho_section(rostr name, int pass, int *bits);
										   ...
										   const char *sectname)
{
	...
	for (s = sects; s != NULL; s = s->next)
		if (!StrCompare(s->segname, segname) && !StrCompare(s->sectname, sectname))
			break;
...
static void macho_init(FILE * fp, ldfunc ldef,
					   evalfunc eval)
{
	char zero = 0;
	...
	StrbuffSend(strs, &zero, sizeof(char));
	strslen = 1;
	
	/* add special symbol for ..gotpcrel */
	macho_gotpcrel_sect = SegAlloc();
	macho_gotpcrel_sect ++;
	ldef("..gotpcrel", macho_gotpcrel_sect, 0L, NULL, false, false, &of_macho64);
}
...
static void sect_write(struct section *sect, const uint8_t *data, uint32_t len)
{
					  ...
					  int pcrel, int bytes, int64_t reloff)
{
	...
	if (pcrel == 0) {
		...
		/* inter-section */
			...
			/* external */
			if (fi == NO_SECT) {
				r->snum = StrpageGet(extsyms, section);
						 ...
						 int32_t section, int32_t wrt)
{
	...
	if (secto == NO_SEG) {
		if (type != OUT_RESERVE)
			aasm_log(_LOG_ERROR, "attempt to assemble code in "
				  "[ABSOLUTE] space");
	...
	if (s == NULL) {
		aasm_log(_LOG_WARN, "attempt to assemble code in"
			  " section %d: defaulting to `.text'", secto);
		s = get_section_by_name("__TEXT", "__text");

		/* should never happen */
		if (s == NULL)
			aasm_log(_LOG_PANIC, "text section not found");
	}

	sbss = get_section_by_name("__DATA", "__bss");

	if (s == sbss && type != OUT_RESERVE) {
		aasm_log(_LOG_WARN, "attempt to initialize memory in the"
			  " BSS section: ignored");
	...
	case OUT_ADDRESS:
		...
		if (section != NO_SEG) {
			if (section % 2) {
				aasm_log(_LOG_ERROR, "Mach-O format does not support"
					  " section base references");
			} else {
				if (wrt == NO_SEG) {
					if (size < 8) {
						aasm_log(_LOG_ERROR, "Mach-O 64-bit format does not support"
	...
	case OUT_REL2ADR:
		...
		if (section == secto)
			aasm_log(_LOG_PANIC, "intra-section OUT_REL2ADR");
		...
		if (section == NO_SEG) {
			...
			aasm_log(_LOG_ERROR, "Unsupported non-32-bit"
	...
	case OUT_REL4ADR:
		...
		if (section == secto)
			aasm_log(_LOG_PANIC, "intra-section OUT_REL4ADR");
		...
		if (section != NO_SEG && section % 2) {
			...
			if (wrt == NO_SEG) {
				*mydata -= add_reloc(s, section, 1, 4, (int64_t)*mydata);				// X86_64_RELOC_SIGNED/BRANCH
				...
				if (s->data->total_len > 1) {
					StrbuffPopx(s->data, s->data->total_len-2, &gotload, 1);				// Retrieve Instruction Opcode
				} else {
					gotload = 0;
				}
	...
	default:
		aasm_log(_LOG_PANIC, "unknown output type?");
		break;
	}
}
...
static int32_t macho_section(rostr name, int pass, int *bits)
{
	...
	if (!name) {
		*bits = 64;
		...
		sectionAttributes = (char*)name;
		name = aasm_strsep(&sectionAttributes, " \t");
	}

	for (sm = sectmap; sm->nasmsect != NULL; ++sm) {
		/* make lookup into section name translation table */
		if (!StrCompare(name, sm->nasmsect)) {
			...
			if (index == -1) {
				...
				xStrCopyN(s->segname, sm->segname);
				xStrCopyN(s->sectname, sm->sectname);
				s->size = 0;
				s->nreloc = 0;
				s->flags = sm->flags;
			...
			while ((NULL != sectionAttributes)
				   && (currentAttribute = aasm_strsep(&sectionAttributes, " \t"))) {
				if (0 != *currentAttribute) {
					if (!StrCompareNInsensitive("align=", currentAttribute, 6)) {
						char *end;
						int newAlignment, value;

						value = StrTokenULong(currentAttribute + 6, (char**)&end, 0);
						...
						s->align = newAlignment;
					} else if (!StrCompareInsensitive("data", currentAttribute)) {
	...
	aasm_log(_LOG_PANIC, "invalid section name %s", name);
	return NO_SEG;
}
						 ...
						 int is_global, char *special)
{
	...
	if (special) {
		aasm_log(_LOG_ERROR, "The Mach-O output format does "
			  "not support any special symbol types");
		return;
	}

	if (is_global == 3) {
		aasm_log(_LOG_ERROR, "The Mach-O format does not "
	...
	if (name[0] == '.' && name[1] == '.' && name[2] != '@') {
		 ...
		 */
		if (StrCompare(name, "..gotpcrel"))
			aasm_log(_LOG_ERROR, "unrecognized special symbol `%s'", name);
		 return;
	}

	sym = *symstail = malcof(struct symbol);
	sym->next = NULL;
	symstail = &sym->next;
	...
	if (section == NO_SEG) {
		...
		if (sym->sect == NO_SECT) {
			...
			extsyms = StrpageSet(extsyms, section, nsyms);
			...
			default:
				 ...
				 ** SegAlloc() on every call for them) */
				aasm_log(_LOG_PANIC, "in-file index for section %d not found",
					  section);
			}
		}
	}
...
{
	return (StrCompare ((*s1)->name, (*s2)->name));
}
	...
	*strtabsize = sizeof (char);
	...
	if ((sym->type & N_EXT) == 0) {
		...
		*numsyms = *numsyms + 1;
		...
		sym->strx = *strtabsize;
		StrbuffSend (strs, sym->name, (int32_t)(StrLength(sym->name) + 1));
		*strtabsize += StrLength(sym->name) + 1;
	}
	symp = &(sym->next);
	}
	...
	extdefsyms = (symbol**)malc(nextdefsym * sizeof(struct symbol *));
	undefsyms = (symbol**)malc(nundefsym * sizeof(struct symbol *));
...
static void macho_write_section (void)
{
	...
	StrbuffRewind(s->data);
	for (r = s->relocs; r != NULL; r = r->next) {
		len = (int32_t)r->length << 1;
		if(len > 4) len = 8;
		StrbuffPopx(s->data, r->addr, blk, len);
		p = q = blk;
		l = *p++;
		...
		*q++ = l & 0xFF;
		...
		StrbuffSendx(s->data, r->addr, blk, len);
	}

	/* dump the section data to file */
	StrbuffSendFile(s->data, machofp);
	}
...
static void macho_write (void)
{
	...
	*/
	...
	offset = macho_write_segment (offset);
		...
		aasm_log(_LOG_WARN, "no sections?");
