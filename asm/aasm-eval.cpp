...
static void addtotemp(int32_t type, int64_t value)
{
    while (ntempexpr >= tempexpr_size) {
        ...
        tempexpr = (expr*)aasm_realloc(tempexpr, tempexpr_size * sizeof(*tempexpr));
    }
...
static expr *finishtemp(void)
{
    ...
    while (ntempexprs >= tempexprs_size) {
        ...
        tempexprs = (expr**)aasm_realloc(tempexprs, tempexprs_size * sizeof(*tempexprs));
    }
...
static expr *segment_part(expr * e)
{
	...
	if (!is_reloc(e)) {
		//--auto clean flag
		aasm_log(_LOG_ERROR, "cannot apply SEG to a non-relocatable value");
        return NULL;
    }
	...
	if (seg == NO_SEG) {
		...
		//--auto clean flag
        aasm_log(_LOG_ERROR, "SEG applied to something which is already a segment base");
        return NULL;
    } else {
 ...
 *
 * eval_evaluate() should report its own errors: on return it is assumed
...
static expr *rexp0(int critical)
{
    ...
    while (i == TOKEN_DBL_OR) {
        ...
        if (!(is_simple(e) || is_just_unknown(e)) ||
			...
			//--auto clean flag
			aasm_log(_LOG_ERROR, "`|' operator may only be applied to scalar values");
        }
    ...
    while (i == TOKEN_EQ || i == TOKEN_LT || i == TOKEN_GT ||
        ...
        default:
            if (is_unknown(e))
				...
				//--auto clean flag
				aasm_log(_LOG_ERROR, "`%s': operands differ by a non-scalar",
...
static expr *expr3(int critical)
{
    ...
    while (i == TOKEN_SHL || i == TOKEN_SHR) {
        ...
        if (!(is_simple(e) || is_just_unknown(e)) ||
			!(is_simple(f) || is_just_unknown(f))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "shift operator may only be applied to scalar values");
...
static expr *expr5(int critical)
{
    ...
    while (i == '*' || i == '/' || i == '%' ||
        ...
        if (j != '*' && (!(is_simple(e) || is_just_unknown(e)) ||
			!(is_simple(f) || is_just_unknown(f)))) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "division operator may only be applied to scalar values");
            return NULL;
        }
		if (j != '*' && !is_unknown(f) && reloc_value(f) == 0) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "division by zero");
            return NULL;
        }
        switch (j) {
        case '*':
            if (is_simple(e))
				...
				//--auto clean flag
				aasm_log(_LOG_ERROR, "unable to multiply two non-scalar objects");
                return NULL;
            }
            break;
        case '/':
...
static expr *eval_floatize(enum floatize type)
{
	...
	if (i != TOKEN_FLOAT) {
		//--auto clean flag
		aasm_log(_LOG_ERROR, "expecting floating-point number");
	return NULL;
    }
...
static expr *eval_strfunc(enum strfunc type)
{
	...
	if (i != TOKEN_STR) {
		//--auto clean flag
		aasm_log(_LOG_ERROR, "expecting string");
		return NULL;
    }
	...
	if (rn_warn) {
		//--auto clean flag
		log_pass1 = 1;
		aasm_log(_LOG_WARN, "character constant too long");
	}
...
static expr *expr6(int critical)
{
    ...
    case TOKEN_SEG:
		...
		if (is_unknown(e) && critical) {
			//--auto clean flag
			aasm_log(_LOG_ERROR, "unable to determine segment base");
            return NULL;
        }
        return e;

    case TOKEN_FLOATIZE:
	return eval_floatize((floatize)tokval->t_integer);

    case TOKEN_STRFUNC:
	return eval_strfunc((strfunc)tokval->t_integer);
	...
	case TOKEN_INSN:
        ...
        case TOKEN_BASE:
			...
			if (!location->known) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "%s not supported in preprocess-only mode",
					(i == TOKEN_HERE ? "`$'" :
						i == TOKEN_BASE ? "`$$'" :
						"symbol references"));
                addtotemp(EXPR_UNKNOWN, 1L);
            ...
            if (i == TOKEN_BASE) {
                ...
                if (!labelfunc(tokval->t_charptr, &label_seg, &label_ofs)) {
					...
					if (critical == 2) {
						//--auto clean flag
						aasm_log(_LOG_ERROR, "symbol `%s%s' undefined", scope,tokval->t_charptr);
                        return NULL;
                    } else if (critical == 1) {
						aasm_log(_LOG_ERROR, "symbol `%s%s' not defined before use", scope, tokval->t_charptr);
                        return NULL;
                    } else {
                        if (opflags)
                            *opflags |= 1;
	...
	default:
		//--auto clean flag
		aasm_log(_LOG_ERROR, "expression syntax error");
        return NULL;
    }
}

void eval_global_info(struct outffmt *output, lfunc lookup_label,
...
expr *eval_evaluate(scanner sc, void *scprivate, struct tokenval *tv, int *fwref, int critical, struct eval_hints *hints)
{
    ...
    if (f) {
        ...
        if (is_just_unknown(f))
			...
			if (!is_reloc(f)) {
				//--auto clean flag
				aasm_log(_LOG_ERROR, "invalid right-hand operand to WRT");
                return NULL;
            }
            value = reloc_seg(f);
            if (value == NO_SEG)
